...
### 📚 解法 — 從部落格淬煉
35 個工具全部有阿凱手寫的部落格長文（總計 6000-17000 字 / 工具），最權威的內容來源。流程：
1. 寫 Node script 從 `posts.ts` 提取每工具對應 `body`，依字數平衡切成 3 批
...
- **#49 教務處寶藏庫**：改正成「academic 子站含 3 子頁」（不是資源庫）

### 📊 數據比較
| | v3.6.57 (subagent 憑空) | v3.6.58 (部落格淬煉) |
|---|---|---|
| 平均字數 | 475 字 | **948 字** (×2) |
