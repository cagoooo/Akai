...
exports.restoreFromSnapshot = exports.dailySnapshot = void 0;
...
async function readAllDocs(collection) {
 ...
 */
exports.restoreFromSnapshot = (0, https_1.onCall)(async (request) => {
    // 必須是 admin custom claim 才能還原
    if (!request.auth?.token?.admin) {
        throw new https_1.HttpsError("permission-denied", "只有管理員可以還原快照");
    }
    const date = request.data?.date;
    const dryRun = request.data?.dryRun !== false; // 預設 dryRun，避免誤觸
    if (!date || typeof date !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
        throw new https_1.HttpsError("invalid-argument", "date 必須是 YYYY-MM-DD 格式");
    }
    const snapDoc = await admin.firestore().collection(SNAPSHOTS_COLLECTION).doc(date).get();
    if (!snapDoc.exists) {
        throw new https_1.HttpsError("not-found", `找不到 ${date} 的快照`);
    }
    const snap = snapDoc.data();
    const restored = {};
    for (const collection of ["visitorStats", "analytics", "toolUsageStats", "toolRatings"]) {
        const docs = snap.data[collection] || {};
...
//# sourceMappingURL=dailySnapshot.js.map