...
exports.embedQuery = (0, https_1.onCall)({
    ...
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "請先登入（含匿名 auth）才能使用語意搜尋");
    }
    // ── 2. 速率限制 ──
    if (!checkRateLimit(uid)) {
        throw new https_1.HttpsError("resource-exhausted", "請稍等 1 分鐘後再試（每分鐘上限 20 次）");
    }
    // ── 3. 參數驗證 ──
    const query = String(request.data?.query || "").trim();
    if (!query)
        throw new https_1.HttpsError("invalid-argument", "query 不可為空");
    if (query.length > 200) {
        throw new https_1.HttpsError("invalid-argument", "query 太長（上限 200 字）");
    }
    ...
    if (!apiKey || apiKey === "PLACEHOLDER_NOT_CONFIGURED") {
        ...
        throw new https_1.HttpsError("failed-precondition", "語意搜尋功能尚未啟用（管理員未設 Gemini API key）");
    }
    ...
    try {
        ...
        if (!Array.isArray(vector)) {
            throw new https_1.HttpsError("internal", "Gemini API 回傳格式異常");
        }
        ...
        throw new https_1.HttpsError("internal", `Embedding 失敗：${msg.slice(0, 100)}`);
    }
});
...
//# sourceMappingURL=embedQuery.js.map