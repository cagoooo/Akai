...
const FUNNEL_EVENTS = new Set([
    "opened", "audienceSelected", "schoolLevelSelected", "teacherRoleSelected",
    "departmentSelected", "painPointsConfirmed", "resultsShown", "dismissed", "reshuffled",
]);
const WIZARD_STEPS = new Set([
    "audience", "school-level", "teacher-role", "department", "pain-points", "thinking", "results",
]);
const SELECTIONS: Record<string, Set<string>> = {
    ...
    painPoints: new Set([
        "lesson-planning", "assessment", "classroom-management", "student-practice",
        "teacher-workload", "communication", "administration", "meeting-productivity",
        "content-creation", "presentation", "language-learning", "reading-literacy",
        "digital-literacy", "creative-learning", "event-management", "it-support",
        "media-production", "professional-learning", "accessibility", "resource-discovery",
    ]),
};
const SLOTS = new Set(["universal", "role", "stage", "popular", "discovery", "painpoint", "unknown"]);
...
function stringValue(value: unknown, maxLength: number): string {
    ...
    if (!clean || clean.length > maxLength) throw new HttpsError("invalid-argument", "invalid string length");
    return clean;
}
...
function incrementMap(values: string[]): Record<string, admin.firestore.FieldValue> {
    return Object.fromEntries(values.map((value) => [value, admin.firestore.FieldValue.increment(1)]));
}
...
async function recordVisitorCount(): Promise<void> {
    ...
    await admin.firestore().collection("visitorStats").doc("global").set({
...
async function recordVisitorContext(data: Record<string, unknown>, isAdmin: boolean): Promise<void> {
    const category = enumValue(data.category, new Set(["device", "referrer", "geo"]), "category");
    const key = stringValue(data.key, 64);
    if (!/^[\p{L}\p{N} _.:/-]+$/u.test(key)) throw new HttpsError("invalid-argument", "invalid context key");
    ...
    const field = category === "device" ? "deviceStats" : category === "referrer" ? "referrerStats" : "geoStats";
    await admin.firestore().collection("analytics").doc("visitorContext").set({
...
async function recordReco(data: Record<string, unknown>): Promise<void> {
    ...
    if (action === "funnel") {
        ...
        const dimension = enumValue(data.dimension, new Set(Object.keys(SELECTIONS).filter((key) => key !== "painPoints")), "dimension");
        const value = enumValue(data.value, SELECTIONS[dimension], "selection value");
        const eventByDimension: Record<string, string> = {
            audience: "audienceSelected", schoolLevels: "schoolLevelSelected",
            teacherRoles: "teacherRoleSelected", departments: "departmentSelected",
        };
        ...
        const painPoints = [...new Set(raw.map((value) => enumValue(value, SELECTIONS.painPoints, "pain point")))].slice(0, 3);
        ...
        const painPoints = [...new Set(rawPains.map((value) => enumValue(value, SELECTIONS.painPoints, "pain point")))].slice(0, 3);
...
async function recordToolIndexQuery(data: Record<string, unknown>): Promise<void> {
    ...
    const ref = admin.firestore().collection("analytics").doc("toolIndexQueries").collection("queries").doc(simpleHash(query));
    await admin.firestore().runTransaction(async (transaction) => {
        const snapshot = await transaction.get(ref);
        const now = new Date().toISOString();
        if (snapshot.exists) {
            transaction.set(ref, {
                count: admin.firestore.FieldValue.increment(1), lastUsedAt: now, lastResultCount: resultCount,
            }, { merge: true });
        } else {
            transaction.set(ref, { query, count: 1, firstSeenAt: now, lastUsedAt: now, lastResultCount: resultCount });
        }
    });
}
...
async function recordWebVital(data: Record<string, unknown>): Promise<void> {
    ...
    if (!/^[a-zA-Z0-9._-]+$/.test(metricId)) throw new HttpsError("invalid-argument", "invalid metric id");
    const value = Number(data.value);
    const delta = Number(data.delta);
    if (!Number.isFinite(value) || value < 0 || value > 120000 || !Number.isFinite(delta) || Math.abs(delta) > 120000) {
    ...
    const navigationType = typeof data.navigationType === "string" ? data.navigationType.slice(0, 40) : "unknown";
    const keepsDecimal = name === "CLS";
    await admin.firestore().collection("analytics").doc("webVitals").collection(todayInTaipei()).doc(metricId).set({
        name,
...
export const recordPublicAnalytics = onCall(
    ...
    async (request) => {
        ...
        const kind = stringValue(data.kind, 32);
        const isAdmin = request.auth?.token?.admin === true;
        switch (kind) {
            case "visitorCount": await recordVisitorCount(); break;
            case "visitorContext": await recordVisitorContext(data, isAdmin); break;
            case "recommendation": await recordReco(data); break;
            case "toolIndexQuery": await recordToolIndexQuery(data); break;
            case "webVital": await recordWebVital(data); break;
            default: throw new HttpsError("invalid-argument", "invalid analytics kind");
        }
        return { ok: true };
    },
);
