...
describe('buildAudienceFunnel', () => {
    ...
    it('依流程順序算出留存與流失', () => {
        ...
        expect(result.steps.map((s) => s.key)).toEqual([
            'opened', 'audienceSelected', 'schoolLevelSelected', 'painPointsConfirmed', 'resultsShown',
        ]);
    ...
    it('後段步驟數字反而較大時（寫入時間差）強制單調遞減，不會出現越走越多人', () => {
        const result = buildAudienceFunnel({ opened: 50, audienceSelected: 40, schoolLevelSelected: 45, painPointsConfirmed: 20, resultsShown: 18 });
        const counts = result.steps.map((s) => s.count);
        expect(counts).toEqual([50, 40, 40, 20, 18]);
    });

    it('樣本太少就不指認最大流失（不拿 3 個人的資料改流程）', () => {
        const result = buildAudienceFunnel({ opened: 5, audienceSelected: 1, schoolLevelSelected: 1, painPointsConfirmed: 1, resultsShown: 1 });
