...
const BRANCH_STEPS: { key: string; label: string; parentKey: string; note: string }[] = [
  {
    key: 'quickPreviewStarted',
    label: '先看熱門工具',
    parentKey: 'opened',
    note: '免填資料快速路徑',
  },
  {
    key: 'quickPreviewResultsShown',
    label: '快速預覽看到結果',
    parentKey: 'quickPreviewStarted',
    note: '快速路徑完成',
  },
  {
    key: 'teacherRoleSelected',
    label: '選了職務',
    parentKey: 'schoolLevelSelected',
    note: '只有老師會走到',
  },
  {
    key: 'departmentSelected',
    label: '選了處室',
    parentKey: 'teacherRoleSelected',
    note: '只有行政人員會走到',
  },
];
...
export function buildAudienceFunnel(
  ...
  const drops = normalized.map((step, index) =>
    index === 0 ? 0 : normalized[index - 1].count - step.count,
  );
