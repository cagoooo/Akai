...
export function AnalyticsDashboard() {
  ...
  const { data: visitorStats } = useQuery<VisitorStats>({
    ...
    queryFn: async () => {
      ...
      if (isStaticDeployment()) {
        console.log('[Static Mode] 跳過訪問統計 API 請求');
    ...
    refetchInterval: isStaticDeployment() ? false : 30000, // 靜態環境不刷新
  });
  ...
  const { data: toolStats } = useQuery<ToolUsageStat[]>({
    ...
    queryFn: async () => {
      // 在靜態部署環境中，返回空數據
      if (isStaticDeployment()) {
        console.log('[Static Mode] 跳過工具統計 API 請求');
        return [];
      }
    ...
    refetchInterval: isStaticDeployment() ? false : 60000, // 靜態環境不刷新
  });
