...
export function AudienceFunnelChart({
  ...
  const { steps, branches, completionRate, hasEnoughSample } = buildAudienceFunnel(
    funnel,
    minSample,
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          個人化主線與免填資料快速預覽分開統計；樣本少於 {minSample}{' '}
          次開啟時只顯示數字，不指認「最該修的一步」。
        </CardDescription>
      </CardHeader>
        ...
        {opened === 0 ? (
          ...
          <>
            ...
            <div className="rounded-md border-2 border-amber-200 bg-amber-50/70 p-3 text-sm">
              ...
              {hasEnoughSample && biggest ? (
                <div className="mt-2 flex items-start gap-2 text-[#7a4a10]">
                  ...
                  <span>
                    下一個該優化的是「<strong>{biggest.label}</strong>」這一步， 在這裡流失了{' '}
                    {biggest.dropped.toLocaleString()} 人（{pct(biggest.dropRate)}）。
                  </span>
                </div>
              ) : (
            ...
            <div className="space-y-1 text-sm">
              <div className="text-[12px] font-bold text-[#6b5a35]">
                分支步驟（免填資料預覽與特定身分流程，不計入上面的個人化流失）
              </div>
              {branches.map((branch) => (
                <div
                  key={branch.key}
