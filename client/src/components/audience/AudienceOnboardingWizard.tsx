...
export function AudienceOnboardingWizard({ open, tools, onComplete, onDismiss, onLocateTool }: Props) {
  ...
  useEffect(() => { if (state.step === 'results' && profile) onComplete(profile); }, [state.step, profile, onComplete]);
  if (!open) return null;
  const heading = state.step === 'audience' ? '先認識你，推薦會更準' : state.step === 'results' ? '為你準備的推薦工具' : '告訴我一點你的工作情境';
  return <div className="audience-wizard" role="dialog" aria-modal="true" aria-label="找到適合你的教育工具" onMouseDown={(e) => { if (e.target === e.currentTarget) onDismiss(); }}>
    <div className="audience-wizard__paper">
      ...
      {state.step !== 'audience' && state.step !== 'results' && <button type="button" className="audience-wizard__back" onClick={() => dispatch({ type: 'BACK' })}><ArrowLeft size={17} /> 上一步</button>}
