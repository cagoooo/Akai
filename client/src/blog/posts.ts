...
const POST_126: BlogPost = {
  ...
  title: '#126 DEADZONE 能量防線：一款「輸了不是因為你被打死」的 3D 塔防',
  excerpt:
    '#126 是一款直接在瀏覽器跑的 3D 塔防射擊：三種能量武器隨時切換、三種防禦塔各有分工、十波殭屍撲向能源核心。最關鍵的設計是角色不會受傷、敵人只攻核心——孩子輸掉的是一個決策，不是一條命，所以他會想再來一局。',
  ...
  coverEmoji: '🎮',
  coverColor: 'green',
  body: `## 為什麼很多課堂遊戲，孩子玩兩次就不玩了

我在資訊課放過不少小遊戲，觀察到一個很一致的現象：**讓孩子放棄的通常不是「太難」，而是「重來的代價太高」。**

角色一碰到就死、死了從頭開始、前面五分鐘的努力歸零——這種設計在遊戲市場上很正常，因為玩家是自願來受虐的。但在四十分鐘的課堂裡，它會製造出很現實的兩極化：反應快的孩子越玩越順，卡關的孩子第三次死掉之後就把視窗關掉，開始找別的事做。而且失敗回饋全部集中在「我手不夠快」，跟我真正想讓他們練的東西——判斷、規劃、取捨——沒什麼關係。

所以當我在做 **DEADZONE｜能量防線** 的時候，第一個定下來的規則不是武器有幾種，而是：**玩家角色不會受傷，敵人只攻擊能源核心。**

## 這個決定改變了整局遊戲的性質

角色不死，代表孩子可以放心走進戰場中間、繞到彎道後面、試試看站在哪個角度射得到最多敵人。他不會因為「站錯位置」被懲罰，只會因為「塔蓋錯位置」而讓核心掉血。

於是失敗訊息從「你死了」變成「你這一波的火力沒有覆蓋到那個彎道」。這是完全不同的兩句話——後者是可以被討論、被修正、被下一局驗證的。遊戲失敗畫面也不是只寫個 GAME OVER，而是會告訴你在第幾波擊退了幾隻，並直接給一句戰術建議：試試在彎道搭配冰凍稜鏡與電漿迫擊塔。
...
**當成課間或獎勵活動**：一局十分鐘剛剛好。因為角色不會死，程度落差大的班級不會有人卡在開頭，願意動腦的孩子則會去挑戰第 5、10 波的巨型殭屍。
