...
→ [前往 #46 禮堂&專科教室&IPAD平板車預約系統](/tool/46)
...
- [#46 禮堂&專科教室&IPAD平板車預約系統](/tool/46) — 同樣「總務數位轉型」系列
...
const POST_102: BlogPost = {
  slug: 'alien-invasion-102-defend-shihmen-space-shooter',
  title: '#102 外星人入侵·保衛石門：復古太空射擊 × 全校排行榜 × 可離線的下課十分鐘',
  excerpt:
    '#102 外星人入侵是阿凱寫給孩子的復古太空射擊遊戲：純 HTML5 Canvas 單檔零相依，三種難度、王關 Boss 母艦、火力道具與連擊加成，石門校徽開場、可加入主畫面當 App、離線也能玩，內建全校跨裝置排行榜，桌機鍵盤 + 手機觸控全支援。',
  publishedAt: '2026-06-03',
  readingMinutes: 5,
  tags: ['太空射擊', '復古像素', '教育遊戲', '全校排行榜', 'PWA 離線'],
  toolIds: [102, 101, 97],
  coverEmoji: '🚀',
  coverColor: 'purple',
  body: `## 下課十分鐘，我想要一個「打開就玩、還能全校拚名次」的遊戲

#101 的 3D 迷宮上線後，班上孩子下課搶著玩，但我發現一件事：**有些孩子想要更直覺、更街機感的東西** — 不用學走位視角、按下去就有雷射、有爆炸、有分數往上跳。

於是我做了 #102：**一款復古像素風的太空射擊遊戲** — 駕駛地球最後一艘戰機，擊退一波波外星艦隊。但這次我給它一個專屬主題：**保衛石門**。開場是石門國小的校徽，標題下面寫著「保衛石門 · DEFEND SHIH MEN」，孩子一打開就知道「這是我們學校的遊戲」。

## #102 怎麼玩？走位 + 撿道具 + 拚連擊

打開 [cagoooo.github.io/alien-invasion](https://cagoooo.github.io/alien-invasion/) 看到的是石門校徽 + 霓虹標題的開始畫面。操作極簡：
...
我刻意做成**自動開火**，因為對低年級或第一次玩的孩子，「一邊移動一邊還要按射擊」太難了。專心走位、閃子彈、瞄準 — 這樣每個孩子都玩得起來。

**核心玩法**：

- 👾 **三種外星人** — 偵察兵（綠、脆）、戰鬥機（藍、會反擊）、轟炸機（粉、肉）。整隊橫向移動、碰到邊緣往下壓，**剩越少打越快**（經典太空侵略者手感）
- 💪 **道具掉落** — 火力強化（單發 → 雙發 → 三連發）、急速射擊、能量護盾、增加戰機
- 🔥 **連擊加成** — 連續擊殺不失誤，分數倍率會往上疊，逼孩子「貪一點但別貪過頭」
- ⚠️ **王關 Boss 母艦** — 每 5 波出現巨型母艦，三種彈幕（雙直射 / 扇形散射 / 瞄準你），上面有血條

## 三種難度，同一間教室不同孩子都玩得起來

這是我做教學遊戲最在意的一點 — **一款遊戲要能涵蓋不同程度的孩子**：

- 🟢 **簡單** — 5 條命、敵人開火慢、移動慢（三四年級、第一次玩）
- 🔵 **普通** — 3 條命、標準節奏（高年級主力）
- 🔴 **困難** — 2 條命、敵彈又快又密（高手 / 資訊老師自己挑戰）

選難度就在開始畫面，三顆按鈕，孩子自己挑。簡單模式讓「手沒那麼快」的孩子也能撐過幾波、嚐到上榜的甜頭；困難模式留給「已經破關十次」的孩子繼續找挑戰。

## 為什麼是「純 HTML5 Canvas 單檔」？

#101 用 React + Three.js，#102 我反其道而行 — **整個遊戲就是一個 \`index.html\`，零相依、零建置**。理由很實際：

**① 任何裝置點開就跑** — 沒有幾百 KB 的 bundle 要下載、沒有框架要載入。學校最老的那台 Chromebook 也是秒開。

**② 改一個數值就上線** — 單檔意味著臨時想調敵人血量、難度節奏，改完 push 就生效，不用等冗長 build。

**③ 教學上是活範例** — 資訊課講「網頁能做什麼」時，我直接打開原始碼給高年級看：「整個遊戲就這一個檔案，用 Canvas 畫的」。孩子看到外星人、爆炸、星空全是程式畫出來的，**對『程式 = 畫圖 + 邏輯』的想像被打開**。

## 加入主畫面當 App、沒網路也能玩

班級平板最怕「要玩還要開瀏覽器打網址」。所以 #102 做成 **PWA**：

- iPad / Chrome / Edge 開啟後選「加入主畫面」→ 變成全螢幕 App，孩子下次點 icon 就開（icon 就是石門校徽）
- 內建 Service Worker 把遊戲快取起來，**沒網路也能玩** — 校外教學遊覽車上、Wi-Fi 斷線的角落都能打一輪

App 圖示是用程式把石門校徽重繪、疊在深空背景上，**任何尺寸都清晰不破圖**。

## 全校排行榜：跨裝置、可留名、健康競爭

這是 #102 跟一般小遊戲最大的差別 — **不是只有自己這台裝置的紀錄，而是全校共用一份**。

遊戲結束輸入暱稱、按「上傳」，分數就進雲端（Firebase 匿名登入 + Firestore），**全校同學在「🌏 全校」分頁看到同一份 TOP 5**。離線時自動退回「📱 本機」榜，連得上線就看全校。

為什麼這對班級經營有用？因為衝高分要**同時顧火力升級、別吃彈、連擊別斷**，不是單純比手速。**策略多元 = 不同特質的孩子都有機會上榜**，而不是手最快的那個壟斷。

## 真實技術棧

- **遊戲核心**：純 HTML5 Canvas 2D + 原生 JavaScript（單檔、零相依、零建置）
- **音效**：Web Audio API 即時合成（雷射、爆炸、道具、Boss 登場）
- **排行榜**：Firebase 匿名登入 + Firestore（跨裝置即時同步）
- **PWA**：manifest.webmanifest + Service Worker（可安裝、離線可玩）
- **校徽**：Canvas 向量重繪石門國小校徽（無外部圖檔、任何尺寸清晰）
- **部署**：GitHub Pages（[cagoooo/alien-invasion](https://github.com/cagoooo/alien-invasion)）

## 桌機 vs 手機，操作有什麼不同？

- **桌機 / Chromebook**：方向鍵或 A / D 移動，自動開火，P 暫停
- **手機 / 平板**：手指在畫面上滑動，戰機跟著手指走

兩端都是「自動開火、只管走位」，從桌機換到平板沒有學習成本，孩子拿起哪台都會玩。

## 老師回饋

> 「孩子一看到開場是**我們學校的校徽**，整個眼睛亮起來。『這是石門的遊戲耶！』那種歸屬感，是隨便一個小遊戲給不了的。」
>
> — 桃園某國小導師

> 「我們班平板沒網路的角落也能玩，校外教學遊覽車上**整車安靜打外星人**，我終於不用一直喊『不要吵』。」
>
> — 帶隊出遊的級任老師

> 「全校排行榜一出來，下課就有孩子衝來問『我全校第幾』。**為了上榜他們會自己研究怎麼撿道具、怎麼連擊**，比我教還認真。」
>
> — 資訊老師

## 跟其他工具的關係：學生自驅體驗，再補一塊

阿凱工具集裡「孩子打開就玩、不用老師全程顧」的自驅型體驗工具，#102 又補上一塊：

- **#50 童樂學園**：學科 × 趣味（廣度）
- **#97 MBTI 校園奇遇記**：個人 × 情境（深度）
- **#101 3D 迷宮冒險**：空間 × 反應（沉浸 3D）
- **#102 外星人入侵**：反應 × 策略（街機 × 全校競技）

#101 和 #102 剛好是一對：**一個 3D 沉浸、一個 2D 街機**，孩子照心情挑。

## 配對工具推薦

- [#101 3D 迷宮冒險遊戲](/tool/101) — 第一人稱 3D 探索，跟 #102 是 3D / 2D 雙生
- [#97 MBTI 校園奇遇記](/tool/97) — 16 型人格情境探索
- [#102 外星人入侵·保衛石門](/tool/102) — 本篇主角

## 適用對象

- **國小導師** — 下課十分鐘 / 校外教學 / 班級獎勵的健康放電方案
- **資訊老師** — 高年級「網頁做遊戲」的 Canvas 活範例
- **帶平板的班級** — 可安裝、可離線，沒網路也能玩
- **想跟全校拚名次的孩子** — 留名上傳，看自己全校第幾
- **石門國小師生** — 開場就是自己學校的校徽，專屬感拉滿

## 想試試？

→ [前往 #102 外星人入侵·保衛石門](/tool/102)

**第一次玩**：選「簡單」，手機直接滑動或桌機按方向鍵，自動開火，撐過幾波撿幾個道具，遊戲結束留個名上傳。

**想拚名次**：升「普通」，把火力升到三連發、別吃彈、連擊別斷，衝進全校 TOP 5。

**找挑戰**：「困難」2 條命、敵彈又快又密，留給已經破關很多次的孩子。

下節資訊課最後 10 分鐘，**把這個丟給孩子當下課放電** — 而且這次，他們守的是石門。
`,
};
...
const POST_104: BlogPost = {
  slug: 'twinkle-hub-104-taiwan-official-data-mcp',
  title: '#104 台灣官方資料庫 Twinkle-Hub：讓 AI 助教「先查官方資料、再回答你」',
  excerpt:
    '#104 介紹一座「資料橋」MCP 服務 Twinkle Hub：把國定假日、農曆節氣、簡繁轉換、郵遞區號、6.4 萬份國考考古題與政府開放資料接進 Claude 等 AI 助教，65 個現成工具，讓 AI 不再「憑記憶」答錯台灣在地問題。申請金鑰、一鍵安裝、重開即用，老師備課出題、學生查資料都更踏實。',
  publishedAt: '2026-06-04',
  readingMinutes: 6,
  tags: ['台灣官方資料', 'MCP 服務', 'AI 助教', '國考考古題', '備課出題'],
  toolIds: [104, 103, 88],
  coverEmoji: '✨',
  coverColor: 'purple',
  body: `## 我請 AI 幫我排行事曆，它把補班日漏了

那天我想偷懶，請 AI 助教幫我列「這學期的國定假日和補班日」做班級行事曆。它答得很流暢、很有自信——**結果補班日漏了一天、連假天數也算錯**。我那時才意識到一件事：**AI 對「台灣會變動的官方資料」其實很不可靠**。

不是它笨，是它在「**憑記憶**」回答。模型訓練到某個時間點就定格了，可是國定假日每年公告、農曆每天在變、郵遞區號改制、考古題年年新增——這些**會動的官方資料**，靠記憶一定會出錯。

所以當我看到 Twinkle 團隊做的這座「資料橋」，眼睛就亮了。我把它收進工具集當 #104：[**台灣官方資料庫 Twinkle Hub**](https://cagoooo.github.io/twinkle-hub-guide/)。

## #104 怎麼解？讓 AI「先查官方來源，再回答」

Twinkle Hub 是一個 **MCP（Model Context Protocol）服務**。講白話，它就是一座橋：**讓 Claude 這類 AI 助教，能在回答前先「打電話」去查台灣官方資料，再把查到的整理給你**——而不是靠舊記憶硬掰。

它一共給 AI **65 個現成工具**，分成九大類，幾乎涵蓋台灣老師最常踩雷的查詢：

- 🗓️ **曆法與行事曆** — 國定假日、補班日、農曆⇄國曆、24 節氣、生肖
- 📝 **國家考試考古題** — **6.4 萬份考卷、32 萬題**可語意搜尋（附 PDF）
- 🔤 **文字小幫手** — 簡體⇄正體互轉、中文數字轉換
- 📮 **地址與郵遞區號** — 3+3 碼查詢、行政區資訊、地址正規化、中英地址互轉
- 🗺️ **地理與防災** — 避難收容所、座標⇄行政區、最近設施查詢
- 📄 **文件處理** — PDF 文字／頁面抽取、網頁轉 Markdown
- ✅ **格式驗證** — 身分證、統一編號、車牌、電話（**純演算法、不碰個資**）
- 🏦 **生活查詢** — 銀行代碼、捷運站與路線
- 📊 **政府開放資料** — 實價登錄、政府採購、公司登記、司法判決、專利

重點是：**這 65 個工具不用你記指令**。你照常用自然語言問，AI 會自己判斷「這題該查哪個工具」，呼叫、查官方、再回答。

## 老師可以這樣直接問

我自己最常用的幾句，都是備課現場真的會問的：

\`\`\`
「幫我列 2026 下學期的國定假日和補班日，做成行事曆」
「把這篇簡體中文教材，一鍵轉成正體中文」
「幫我找 5 題國中會考相關的考古題，附出處」
「石門國小的郵遞區號是幾號？完整 3+3 碼」
「農曆四月十八換成國曆是哪天？生肖屬什麼？」
\`\`\`

以前這些我得自己開五個網站查。現在一句話丟給 AI 助教，它查官方、整理好回給我，**而且這次補班日不會再漏了**。

## 為什麼是「MCP 服務」，不是一個網站？

這是 #104 跟我其他工具最不一樣的地方——**它不是一個你開來點的網頁，而是一個「裝在 AI 裡」的能力**。

一旦裝好，你用的還是原本那個 Claude / AI 助教，介面完全沒變，只是它**突然變得很懂台灣**。這個「一頁讀懂的介紹頁」就是阿凱幫大家把這件事講清楚：什麼是資料橋、對老師學生家長各有什麼好處、怎麼三步驟裝起來。

## 怎麼開始？三步驟

頁面把安裝流程拆成傻瓜三步，連沒裝過 MCP 的老師都跟得上：

1. **申請鑰匙** — 到 hub.twinkleai.tw/dashboard 申請一把 API 金鑰
2. **一鍵安裝** — 下載 \`twinkle-hub.mcpb\` 安裝包，Claude 桌面版雙擊即裝
3. **重開後開講** — 重開 AI 客戶端，就能開始問台灣的問題了

**基本查詢免費**，部分進階的開放資料服務可能有用量限制。對國小現場來說，日常那些行事曆、簡繁、考古題查詢，免費額度綽綽有餘。

## 隱私這關，它做對了

我會收進工具集、敢推薦給家長，很大一個原因是它的**隱私設計**：那些「身分證 / 統編 / 車牌格式驗證」全是**純演算法檢查格式對不對**，**不連線、不查、不存任何個人資料**。驗證「這張身分證格式合不合規則」跟「查這個人是誰」是兩回事——它只做前者。這點對在學校環境用 AI 很關鍵。

## 真實技術棧

- **協定**：MCP（Model Context Protocol）— 相容任何支援 MCP 的 AI 客戶端
- **安裝**：Claude 桌面版 \`.mcpb\` 一鍵安裝包，雙擊即裝
- **金鑰**：API Key 制（hub.twinkleai.tw/dashboard 申請）
- **資料量**：65 個工具 / 9 大類 / 6.4 萬份考古題 / 32 萬題
- **介紹頁**：純靜態 HTML + PWA（可加到主畫面、離線可看），GitHub Pages 部署
- **團隊**：台灣 Twinkle AI；介紹頁由石門國小阿凱老師整理推薦

## 老師回饋

> 「我終於不用一邊問 AI、一邊自己再開政府網站對一次。**它查的就是官方來源**，行事曆一次就對。」
>
> — 桃園某國小級任老師

> 「出題找考古題超有感，**6 萬多份卷、語意搜尋**，輸入單元名稱就撈得到相關題目，附 PDF 直接用。」
>
> — 準備校內評量的高年級老師

> 「我在資訊課拿它當教材——讓孩子看到『AI 不是什麼都知道，它要會去查工具』，**這個觀念比會用更重要**。」
>
> — 上 AI 素養課的資訊老師

## 跟其他工具的關係：都在「擴充 AI 的能力」

阿凱工具集裡有一條「讓 AI 更能幹」的線，#104 又補上關鍵一塊：

- **#103 AI Agent 的 Web 技能雙引擎**：教 AI 怎麼「動手操作網頁」
- **#104 Twinkle Hub**：給 AI 一座「台灣官方資料庫」可以查
- **#88 課程計畫 AI 審查**：讓 AI 幫老師看課程計畫合不合規

#103 和 #104 剛好是一對：**一個讓 AI 會做事、一個讓 AI 查得到正確的台灣資料**，合起來才是真正好用的台灣 AI 助教。

## 配對工具推薦

- [#103 AI Agent 的 Web 技能雙引擎](/tool/103) — AI 怎麼動手操作網頁，跟 #104 是「會做事 × 查得到」雙生
- [#88 課程計畫 AI 審查工具](/tool/88) — 上傳 PDF 依教育局標準逐項自動比對
- [#104 台灣官方資料庫 Twinkle Hub](/tool/104) — 本篇主角

## 適用對象

- **要排行事曆、出考題的老師** — 國定假日、補班日、考古題一句話查到
- **教 AI 素養的資訊老師** — 「AI 該去查工具，不是憑記憶」的活教材
- **常踩台灣在地資料雷的人** — 郵遞區號、農曆、簡繁轉換不再出錯
- **家長** — 純演算法驗證、不碰個資，可以放心讓孩子用
- **想入門 MCP 的人** — 從一個有完整中文介紹、三步驟就裝好的服務開始

## 想試試？

→ [前往 #104 台灣官方資料庫 Twinkle Hub](/tool/104)

**先看懂**：打開介紹頁，三分鐘搞清楚「資料橋」是什麼、對你有什麼用。

**再動手**：照三步驟——申請鑰匙 → 下載 \`twinkle-hub.mcpb\` 一鍵安裝 → 重開 AI 客戶端。

下次再請 AI 幫你排行事曆，**它會先去查官方公告，再回答你**——這一次，補班日不會再漏了。讓 AI 助教更懂台灣，從今天開始。
`,
};

const POST_106: BlogPost = {
  slug: 'graduation-103-shimen-wishwall-106',
  title: '#106 石門國小第103屆畢業典禮：當家長的祝福，能在三秒內飛上禮堂大螢幕',
  excerpt:
    '每年畢業季，家長最想說的話常常來不及說出口。#106 是石門國小阿凱老師為第103屆畢業典禮做的數位主場：掃 QR 留言、老師 LINE 一鍵審核，祝福三秒飛上禮堂大螢幕的星空投影，家長剛寫完抬頭就看到自己。一條 GAS + LINE 的零成本即時互動閉環。',
  publishedAt: '2026-06-05',
  readingMinutes: 6,
  tags: ['畢業典禮', '即時祝福牆', '星空投影', 'GAS 後端', '掃碼互動'],
  toolIds: [106, 62, 40],
  coverEmoji: '🎓',
  coverColor: 'blue',
  body: `## 畢業典禮上，最珍貴的話常常來不及說

每年畢業季，我都會看到同一個畫面：典禮結束、紅毯走完、大合照拍完，家長拉著孩子在禮堂門口，眼眶紅紅的，想說點什麼，卻只擠得出一句「畢業快樂」。

不是不想說，是**沒有一個地方好好說**。典禮流程很滿、麥克風輪不到家長、孩子六年來的點滴塞在心裡，一句「加油」就帶過了。那些「我看著你從幼稚園走到今天」的話，最後都沒能說出口。

今年石門國小第103屆畢業典禮，我決定做一件事：**讓每一句來不及說的祝福，都有地方落腳，而且能在三秒內飛上禮堂的大螢幕。**

於是有了 #106：[石門國小 第103屆畢業典禮網站](https://cagoooo.github.io/smes-graduation-103/)。

## #106 怎麼解？一條「掃碼就能上大螢幕」的祝福閉環

這個網站表面是畢業典禮的數位主場——典禮資訊、YouTube 直播、校園巡禮、畢業歌〈風箏〉、班導師專區一站到齊。但它真正的靈魂，是一條我來回打磨十幾版才做順的**即時互動閉環**：

**第一步：掃碼留言** — 禮堂大螢幕角落放著 QR Code（可放大好掃），家長手機一掃，開啟祝福表單，三十秒寫完一句話。

**第二步：LINE 一鍵審核** — 祝福送出後寫進 Google Sheet，我的 LINE 立刻收到一張卡片通知，看一眼內容沒問題，**點一下「通過」**。

**第三步：即時上牆** — 通過的瞬間，祝福飛上禮堂大螢幕的**星空投影**（最新的排最前面），家長剛寫完抬頭，就看到自己的話在流星與彩虹之間閃爍。

那種「我寫的話，現在全禮堂都看得到」的感動，是傳統典禮給不了的。

## 不只一種祝福牆

祝福牆我做了三種版本，各有各的場合：

- **主頁分頁牆** — 平常瀏覽，每頁六則、熱門優先
- **放大沉浸牆** — 點開全螢幕，背景透出六班導師的合影
- **現場星空投影版** — 典禮現場專用，流星動畫 + 〈風箏〉BGM + 班級篩選 + 掃碼入口

而且**未成年學生的名字會自動去識別化**（王○明），保護孩子隱私；但給老師、給校友的祝福則完整顯示——因為「謝謝陳老師」這種話，遮起來反而失了溫度。

## 真實技術棧

- **前端**：純靜態 HTML/CSS/JS + GitHub Pages（零成本部署）
- **後端**：Google Apps Script + Google Sheet 當資料庫（老師打開試算表就能看所有祝福）
- **通知／審核**：LINE Messaging API 卡片，一鍵 moderate
- **體驗**：PWA Service Worker、schema.org Event 結構化資料、社群分享 OG 圖、全裝置 RWD

整套**零維運、零月費**，一台筆電 + 一個 Google 帳號 + 一個 LINE Bot 就跑起來了。

## 老師與家長的回饋

> 「我媽媽不太會用智慧型手機，但掃個 QR 填祝福她竟然自己就會了。她寫的那句『阿嬤以你為榮』飛上大螢幕的時候，我女兒哭了。」
>
> — 六年級畢業生家長

> 「以前畢業典禮的祝福都是事後才整理，這次是**即時的**。孩子表演完下台，馬上能看到台下家長剛寫的加油，氣氛完全不一樣。」
>
> — 協助典禮的導師

## 跟其他工具的關係：學校大型活動的「即時互動」這條線

阿凱工具集裡，有一條「**讓學校活動不只是單向廣播**」的線：

- **#62 親職教育日官網**：把一整天的親師活動搬上線
- **#40 Padlet 行政宣導動態牆**：讓訊息變成可互動的牆
- **#106 畢業典禮即時祝福牆**：讓家長的祝福三秒上大螢幕

一個比一個更「即時」、更「雙向」——這正是我想帶給校園活動的改變。

## 配對工具推薦

- [#62 桃園市龍潭區石門國民小學114學年度親職教育日](/tool/62) — 同樣是學校大型活動的數位官網，可一起參考活動頁設計
- [#40 Padlet行政宣導動態牆](/tool/40) — 即時動態牆，跟畢業祝福牆是同一種「集眾人之聲」的概念
- [#106 石門國小 第103屆畢業典禮](/tool/106) — 本篇主角

## 適用對象

- **要辦畢業典禮／大型活動的學校** — 一套就搞定資訊、直播、互動
- **想讓家長即時參與的老師** — 掃碼留言 + 即時上牆的閉環可直接複製
- **想學「GAS + LINE 免費後端」的人** — 零成本做出有審核、有通知的互動系統
- **關心校園數位轉型的人** — 看一場典禮如何從「單向流程」變成「雙向共創」

## 想試試？

→ [前往 #106 石門國小 第103屆畢業典禮](https://cagoooo.github.io/smes-graduation-103/)

**先逛逛**：滑到祝福牆，看看家長與親友寫給畢業生的話；點開「現場星空投影版」，感受流星之間的祝福流動。

**再想想**：如果你的學校下個活動，也能讓每個人的聲音三秒上大螢幕，會是什麼樣子？

畢業典禮會散場，但那些終於說出口的祝福，留在了雲端，也留在了孩子心裡。啟程・感恩・祝福，從石門出發。
`,
};
...
const POST_108: BlogPost = {
  ...
  excerpt:
    '資安宣導最怕變成「老師念一遍、學生左耳進右耳出」。#108 是阿凱老師做的資訊素養情境闖關：學生扮演「鱻盾資安特攻隊」資安新兵，跟 NotebookLM 的阿盾教官對話，一封封判斷訊息是安全還是釣魚、說出破綻、集滿 5 枚徽章晉升正式隊員。用「五看口訣」拆穿詐騙，一節課把防身術練進肌肉記憶。',
  publishedAt: '2026-06-12',
  readingMinutes: 6,
  tags: ['資安教育', '反詐騙', '數位素養', '五看口訣', 'NotebookLM'],
  toolIds: [108, 63, 97],
  coverEmoji: '🛡️',
  coverColor: 'blue',
  body: `## 資安課最尷尬的那十分鐘

每學期總有那麼一節「資訊安全宣導」。我站在台上念：「不要點來路不明的連結、密碼不要外洩、看到中獎訊息不要信⋯⋯」台下三十雙眼睛，有的放空、有的偷滑手機——諷刺的是，他們手機裡此刻可能正躺著一封釣魚簡訊。

問題不在孩子不認真，而在**「念注意事項」這件事本身就是反效果**。資安不是用聽的，是要用「判斷」的。你得親手拆過幾封假銀行、假老師、假官方的訊息，才會在下次真的遇到時，心裡警鈴大作。

所以我把這節課，做成了一場闖關遊戲：[資安特攻隊：釣魚郵件偵蒐](https://cagoooo.github.io/security-squad-cockpit/)。

## #108 怎麼玩？學生扮新兵，跟 AI 教官對話闖關

打開網站，學生不是「上課」，而是**入伍**——成為「鱻盾資安特攻隊」的資安新兵（「鱻盾」呼應我們石門國小的鱻魚特色學園，孩子一看就有歸屬感）。

點「開始闖關」，進入 NotebookLM 聊天室，向**阿盾教官**報到。接下來的循環很簡單，卻很上癮：

1. **接收可疑訊息** — 教官把攔截到的一封訊息交給你
2. **判斷真假** — 這是 ✅ 安全，還是 ⚠️ 釣魚陷阱？
3. **說出破綻集徽章** — 講得出哪裡露馬腳，才拿得到徽章。卡關了可以說「請求提示」，教官會給線索不直接給答案
4. **晉升** — 集滿 **5 枚資安徽章**，從新兵晉升「鱻盾正式隊員」

關鍵在於：對話是 **NotebookLM AI 即時生成的**。每個孩子的闖關節奏、收到的訊息、教官的回應都不一樣，像真的有個教官在旁邊一對一帶——這是 NotebookLM 少見、但我覺得最迷人的用法：**不是拿來生 Podcast 或簡報，而是當一個會出題、會引導、會即時回饋的 AI 家教。**

## 核心武器：辨識釣魚的「五看口訣」

闖關前，全班先把這句口訣記起來，遇到可疑訊息照著看一遍，破綻無所遁形：

- **一看寄件人** — 真公司不會用免費信箱寄正式通知
- **二看語氣** — 正派機構不會恐嚇你「不處理就停權」
- **三看連結** — 滑鼠移上去，驗證真實網址是不是你以為的那個
- **四看要求** — 任何叫你交出密碼、OTP 驗證碼的，一律拒絕
- **五看好康錯字** — 太好康 ＋ 錯字連篇 ＝ 詐騙無誤

簡單五句，卻是孩子一輩子帶得走的防身術。

## 不只揪釣魚，帶走四個數位防身術

闖完這場，小隊員會帶走四個能力：**辨識網路釣魚**、**密碼安全四守則**（夠長、不重複、不外洩、開兩步驟驗證）、**保護個人資料**（送出前先想：對方為什麼需要？管道可信嗎？）、以及**詐騙應變**——萬一上當，先告訴大人、立刻改密碼、撥打 **165** 反詐騙專線。

## 老師端：一節課就能開課

我最在意的是「別的老師也能直接拿去上」。所以 #108 把老師要的都備好了：

- **老師導覽五步驟**：暖身念口訣（5 分）→ 師生一起示範破解案件 1（5 分）→ 兩三人小組各自闖關（20 分）→ 對照學習總表收斂分享（5-10 分）→ 叮嚀密碼保密與 165
- **NotebookLM 自動生成教材**：一份「遊戲說明書」簡報、一份「核心學習內容總表」資料表
- **上課互動工具**：右下角內建 ✏️ 畫筆標註（圈重點）、🎲 隨機抽號器（點組別發表）

一節課（30-40 分鐘）剛剛好，不用準備投影片、不用印學習單，打開網址就能教。

## 真實技術棧

- **前端**：純靜態 HTML/CSS/JS + GitHub Pages 零成本部署
- **闖關引擎**：NotebookLM 當即時 AI 教官，對話、出題、給提示全由它生成
- **定位**：IT COCKPIT 資訊素養駕駛艙系列的一員，跟 [#81 教學駕駛艙入口網](/tool/81) 同源

零後端、零月費，一個 NotebookLM 筆記本 + 一頁靜態網站就跑起來了。

## 跟其他工具的關係：把「數位素養」玩起來

阿凱工具集裡，有一條「**把硬邦邦的數位素養變好玩**」的線：

- **#63 Email 帳密記憶遊樂園**：用遊戲練帳號密碼的安全習慣
- **#97 MBTI 校園奇遇記**：用情境闖關帶孩子認識自己
- **#108 資安特攻隊**：用對話闖關練成反詐騙的本能

它們都不說教，而是讓孩子在「玩」的過程裡，把該懂的事內化成直覺。

## 配對工具推薦

- [#63 Email 帳密記憶遊樂園](/tool/63) — 同樣是數位資安主題的遊戲，跟釣魚偵蒐是最佳搭檔，一個練辨識、一個練帳密習慣
- [#97 MBTI 校園奇遇記](/tool/97) — 同樣「情境闖關 ＋ 老師生態系」格式，自我探索版的姊妹作
- [#108 資安特攻隊：釣魚郵件偵蒐](/tool/108) — 本篇主角

## 適用對象

- **要上資訊安全 / 數位素養的老師** — 一節課現成教材，打開就能教
- **想帶反詐騙宣導的導師 / 家長** — 比念注意事項有效一百倍
- **想學「NotebookLM 當互動 AI 家教」的人** — 看 NotebookLM 不只生 Podcast，還能當會出題的教官
- **關心孩子上網安全的家長** — 陪孩子一起闖關，順便聊聊家裡收過哪些詐騙訊息

## 想試試？

→ [前往 #108 資安特攻隊：釣魚郵件偵蒐](https://cagoooo.github.io/security-squad-cockpit/)

**先試試**：自己先當一次新兵，跟阿盾教官玩幾關，感受「親手揪出釣魚信」跟「聽老師念注意事項」差多少。

**再想想**：下次孩子手機收到「您的包裹配送異常，請點此更新地址」，他會直接點下去，還是先在心裡跑一遍「五看」？這場闖關，練的就是那一秒的遲疑——而那一秒，往往就是被騙與沒被騙的差別。
`,
};

const POST_107: BlogPost = {
  slug: 'edu-cloud-account-updater-107-openid-roster',
  title: '#107 每學年初最磨人的雜事：上千個教育雲帳號，這次三步驟自動對齊',
  excerpt:
    '每到新學年，承辦老師都要面對同一場惡夢：把全校教育雲帳號總表跟教育局新名冊重新對齊——誰轉走、誰轉進、誰升年級換了班級座號，一格一格手動圈、手動貼。#107 是阿凱老師為石門國小做的純前端工具：上傳舊帳密總表與教育局新名冊，自動比對轉入轉出、揪出末四碼撞號、跨學年保留帳密只更新座號，一鍵產出總表、異動清冊與新生通知單。全程在瀏覽器運算，個資一個 byte 都不上傳。',
  publishedAt: '2026-06-12',
  readingMinutes: 6,
  tags: ['教育雲帳號', '校務行政', '末四碼撞號', '純前端處理', '個資不上傳'],
  toolIds: [107, 72, 2],
  coverEmoji: '🏫',
  coverColor: 'green',
  body: `## 開學前那箱沒人想接的差事

每年八月底，學校總務或資訊組的桌上都會出現同一份檔案：「學生教育雲帳密總表」。它躺在 NAS 上，一年級到六年級分成好幾個工作表，記著全校上千名學生的教育雲帳號。

教育雲帳號是教育部給每個學生的 OpenID——**帳號就是身分證末四碼，密碼就是西元出生年月日 8 碼**。平常沒人會去動它，但一到新學年，承辦老師就得做一件超級瑣碎、又超級容易出錯的事：

把這份**舊總表**，跟教育局系統剛下載的**新名冊**重新對齊。

聽起來簡單，做起來要命。誰六年級畢業轉走了？誰是今年新報到的新生？誰升年級重新編了班、座號全變了？這些都要一個一個比對、一格一格貼。一個學校上千名學生，眼睛盯到脫窗，還可能把某個孩子的班級貼錯一格——而那一格錯了，他開學就登不進教育雲。

我看著同事每年開學前對著兩個 Excel 視窗來回切換、用螢光筆在印出來的名單上打勾，就決定：**這種純粹比對的苦工，應該交給電腦。**

於是有了 #107：[學生教育雲帳號更新工具](https://cagoooo.github.io/edu-cloud-updater/)。

## #107 怎麼解？三步驟，把「人工比對」變「自動對齊」

整個工具就是一條清楚的三步驟流水線，承辦老師打開網頁照著做就好：

**第一步：載入現有帳密總表** — 把 NAS 上那份「學生教育雲帳密」檔丟進去（吃 .xls 也吃 .xlsx，一到六年級多個工作表一次讀完）。

**第二步：載入教育局新名冊** — 把教育局系統下載的「學生資料概況_xxxx.xls」丟進去。工具會自動辨識證照號碼、取出末四碼、認出出生日期格式，不用你手動整理欄位。

**第三步：一鍵比對** — 立刻跳出三個數字：**不變、轉出、轉進**。誰要移除、誰是新生，分區列得清清楚楚。

原本要對一整個下午的差事，現在拖兩個檔案、按一下，三秒看完。

## 三個「魔鬼藏在細節裡」的功能

如果只是比對名單，那不算神器。#107 真正花心思的是三個最容易出錯的眉角：

**末四碼撞號偵測** — 教育雲帳號就是身分證末四碼，全校上千人，**難免有兩個孩子末四碼一模一樣**。一旦撞號，帳號就衝突了。工具會自動把所有撞號的組合揪出來提示你人工處理，不會讓它默默地壞掉。

**疑似同一人判定** — 有些孩子改了名字，或證號做了更正。如果工具笨笨地比對，會把他算成「一個轉出 ＋ 一個轉入」，帳密就斷了。#107 會偵測這種情況，跳出來問你「這是不是同一人？」，勾下去就保留原本的帳密，不會無端幫孩子換帳號。

**跨學年升級同步** — 這是我最在意的一條。升年級、重新編班時，勾一下「同步年級／班級／座號」，工具會依新名冊更新每個孩子的班級座號位置，但**帳號（末四碼）和密碼（生日）一律原封不動**。只動位置、不動帳密——孩子升上五年級換了班，但他去年記住的帳密照樣能登入。

主鍵還能切嚴格度：預設「姓名＋末四碼」，遇到同名同末四碼的，再加上「＋生日」就不會誤判。

## 一鍵產出四種成品，連交接都省了

比對完不是給你看看就算了，#107 直接幫你把後續文書全產好：

- **更新後完整總表** — 含六年級工作表、按班級座號排序，下載回存 NAS 就完工
- **異動清冊（.xlsx）** — 摘要／轉入／轉出／注意事項四張表，存查、交接一份搞定
- **異動摘要純文字** — 一鍵複製，直接貼進處室 LINE 群組或公文
- **新生帳密通知單** — A4 多張一頁，可列印也可存 PDF，自動署名「桃園市龍潭區石門國民小學」，發給新生家長就知道帳密怎麼用

承辦老師最怕的「交接時講不清楚」，這下有一份清清楚楚的異動清冊背書。

## 真實技術棧：為什麼堅持「純前端」

- **前端**：純靜態 HTML/CSS/JS + GitHub Pages 零成本部署
- **Excel 引擎**：ExcelJS 直接在瀏覽器裡讀寫 xlsx，不經過任何後端
- **最關鍵的一點**：**所有資料只在你這台電腦的瀏覽器運算，全校學生的姓名、身分證末四碼、生日，一個 byte 都不會上傳到任何伺服器**

學生個資是校園資安最敏感的一塊。我刻意不做後端、不做帳號登入、不存任何資料——你關掉分頁，所有資料就跟著消失。零後端、零月費、零外洩風險，這才是處理孩子個資該有的樣子。

## 跟其他工具的關係：把「行政苦工」一件件自動化

阿凱工具集裡，有一條安靜但很實在的線——**專門替學校行政承辦老師，把那些瑣碎、重複、容易錯的苦工自動化**：

- **#72 動支及黏存單自動產生系統**：把核銷文書自動產出，不再手刻
- **#2 行政業務協調系統**：把處室公告與行事從 LINE 洗版裡救出來
- **#107 學生教育雲帳號更新工具**：把上千個帳號的開學對齊，從一下午變三秒

它們都不華麗，但每一個都替某位承辦老師，省下了一整段本來要熬夜加班的時間。

## 配對工具推薦

- [#72 動支及黏存單自動產生系統](/tool/72) — 同樣是純前端把繁瑣行政文書自動產出，行政承辦的另一根救命稻草
- [#2 行政業務協調系統](/tool/2) — 校內行政數位轉型的同一條線，公告行事一站搞定
- [#107 學生教育雲帳號更新工具](/tool/107) — 本篇主角

## 適用對象

- **學校資訊組 / 總務 / 教務承辦老師** — 每學年初要更新教育雲帳號總表的那個人
- **要發新生帳密通知單的導師** — 一鍵產出 A4 通知單，不用自己排版
- **在意校園資安與個資的人** — 看一個處理上千筆學生個資的工具，如何做到「完全不上傳」
- **想學「純前端讀寫 Excel」的人** — ExcelJS 在瀏覽器端比對、產表的實戰範例

## 想試試？

→ [前往 #107 學生教育雲帳號更新工具](https://cagoooo.github.io/edu-cloud-updater/)

**先試試**：隨手準備兩個 Excel（舊總表、新名冊），拖進去按比對，看它怎麼幫你把轉入轉出、撞號、更名一次標出來。

**再想想**：學校裡還有多少件像這樣「每年都要做一次、純粹比對的苦工」？也許下一個被自動化的，就是你桌上那箱檔案。開學前的加班，可以少一個晚上。
`,
};

const POST_105: BlogPost = {
  slug: 'taiwan-sovereign-ai-corpus-105-shimen-edu',
  title: '#105 臺灣主權 AI 訓練語料庫 × 石門國小：把「國家拿去訓練 AI 的語料」變成孩子的教材',
  excerpt:
    '數位發展部花十年、集 265 個機關蒐集了 6 億 tokens 的臺灣本土語料，本來是拿去餵 AI 模型的。#105 是石門國小阿凱老師把它「教育轉化」的共學站：九大主題、4,554 筆語料一鍵探索，每筆標課綱與來源，還能用自備的免費 Gemini 生成台語／客語學習單與繪本腳本。國家的語料，第一次真正走進國小教室。',
  publishedAt: '2026-06-04',
  readingMinutes: 6,
  tags: ['臺灣主權 AI', '本土語料庫', '數位發展部', '台語客語', '課綱對應'],
  toolIds: [105, 104, 88],
  coverEmoji: '🗺️',
  coverColor: 'green',
  body: `## 一個價值連城的資料庫，卻沒人想到拿來教書

去年底我看到一則新聞：數位發展部推出「臺灣主權 AI 訓練語料庫」，花了**十年**、串接 **265 個機關**，蒐集了 **6 億 tokens** 的正體中文本土語料。海洋、本土語、在地文化、歷史文物、法律判決⋯⋯全是「最台灣」的高品質文本。

新聞底下大家在討論的是：「這樣訓練出來的 AI 會不會更懂台灣？」我腦中冒出來的卻是另一個問題——

**這麼大一座本土語料庫，除了拿去餵模型，難道不能拿來「教書」嗎？**

市面上的 AI 多半有股「美國味」。當我教孩子用 AI，卻發現它不太會講台語、不懂在地廟會、講到海洋總是講錯潮間帶。我們石門國小以「**鱻魚特色學園**」聞名、緊鄰海洋，最適合從海洋與本土教育切入——可是手邊的 AI 工具，偏偏最不懂這些。

那座國家級語料庫，明明就是答案。只是它躺在那裡，是一座給工程師看的「冷資料」，不是給老師用的教材。於是我動手把它**教育轉化**，做成 #105：[**臺灣主權 AI 訓練語料庫 × 石門國小**](https://cagoooo.github.io/taic-edu/)，也就是我們的「鱻本土 AI 共學站」。

## #105 怎麼解？把「冷語料」重新編成「熱教材」

原始語料庫是給 AI 訓練用的，欄位、格式、授權全是工程思維。我做的事，是幫它**換一套老師看得懂的衣服**：

- 把 4,554 筆語料重新歸進**九大國小教育主題**（海洋、本土語、在地文化、交通安全、媒體素養⋯）
- 每一筆都補上**建議年段**（低／中／高年級）
- 每一筆都標**課綱領域與議題對應**，備課時直接對得上
- 每一筆都註明**授權狀態與政府來源**，引用得安心、查得到出處

換句話說：同一批語料，工程師看到的是訓練資料，**老師看到的是「點得到、用得懂、查得到出處」的素材庫**。這就是「教育轉化」四個字的重量。
...
對國小現場來說，它同時是一座**超大型的本土素材寶庫**——你想得到的台灣主題，幾乎都查得到對應語料。

## 不只給老師：師生家長三方共學

我希望它不是「老師備課完就關掉」的工具，而是一家人都用得上的平台：

- **老師** — 素材庫快速篩選、AI 生成教材（繪本、台語朗讀稿、探究提問）、課綱對應、跨校共享
- **學生** — 家鄉探究式自主學習、本土語闖關、跟 AI 共創在地故事繪本、順便養資訊素養
- **家長** — 親子共讀素材、交安與媒體素養延伸、孩子的學習可視化、而且**每份素材都追得到政府來源**

## 兩階段：先能查，再能生成

我把開發拆成兩階段，現況就能用、未來更強：

**階段一・語料導覽站（現在就能用）**：九大主題互動探索器、4,554 筆全站搜尋、開放／需申請一眼分辨、課綱領域與議題對應。

**階段二・本土 AI 教學助理（進行中）**：
- **問 AI** — 依「主題／年段／領域」提問，系統從 4,554 筆台灣語料檢索，**標註引用來源**
- **生成學習單** — 自動產台語／客語／國語版本，分低中高年級，可調 4–8 頁
- **生成繪本腳本** — 輸入領域與課綱單元，自動產出繪本內容結構
- **語料配對（免金鑰）** — 輸入課綱單元或關鍵字，立即撈出相關素材

最讓我有感的是「生成學習單」一鍵出**台語、客語版本**——這在別的 AI 工具上幾乎做不到，因為它們的語料裡根本沒有夠多本土語文本。#105 有，因為它站在國家語料庫的肩膀上。

## 隱私與授權，這關我特別在意

要敢推薦給家長、敢在校園用，隱私和授權一定要先講清楚：

- AI 功能採**使用者自備免費 Gemini 金鑰**，資料只存在你自己的瀏覽器（localStorage），**不上傳、不共用**，免費額度約每天 1,500 次，日常備課綽綽有餘
- **開放資料約 1,154 筆**：加註來源即可用於教學與公開分享
- **需申請約 3,400 筆**：用於 AI 訓練與校本應用，引用前先確認授權範圍
- 頁面也誠實標註：**AI 生成內容僅供參考，請老師審閱後再使用**

把「能開放的」和「要申請的」分得清清楚楚，老師才不會踩到授權的雷。

## 真實技術棧

- **前端**：純靜態 HTML + CSS，九大主題互動探索器、全站語料搜尋
- **AI**：前端直連 Gemini API（使用者自備金鑰），**零後端、零成本**
- **部署**：GitHub Pages
- **資料**：數位發展部「臺灣主權 AI 訓練語料庫」教育轉化版
- **主辦**：數位發展部、桃園市龍潭區石門國民小學（鱻魚特色學園）
- **核心開發**：石門國小阿凱老師

## 為什麼是石門國小做這件事

說真的，國家語料庫誰都能用，但「把它變成國小教材」需要一個**真的站在講台上的人**來做。我每天面對的就是石門的孩子、龍潭的海岸、本土語課的窘境，我知道老師缺什麼、學生卡在哪、家長想看什麼。

這不是一個工程 demo，是一個**老師寫給老師的工具**。國家把語料準備好了，我負責把它接到教室的最後一哩路。

## 老師回饋

> 「我第一次能一鍵生出**台語版**的學習單，而且底下還附語料來源。本土語課終於有像樣的素材了。」
>
> — 桃園某國小本土語支援老師

> 「九大主題探索器我直接拿來當備課入口，輸入單元就撈到對應語料，**省掉我自己上五個政府網站翻資料**的時間。」
>
> — 準備海洋教育課的中年級老師

> 「最打動我的是『每筆都查得到政府出處』。教媒體素養時，我可以直接示範給孩子看『資料要追到源頭』。」
>
> — 上媒體素養課的高年級導師

## 跟其他工具的關係：讓 AI 真正「懂台灣」

阿凱工具集裡有一條「**讓 AI 更懂台灣**」的線，#105 補上了最關鍵的「資料源頭」：

- **#104 台灣官方資料庫 Twinkle Hub**：讓 AI 即時查台灣官方資料（假日、農曆、地址）
- **#105 臺灣主權 AI 訓練語料庫**：讓 AI 有一座**國家級本土語料**可以學、可以生成教材
- **#88 課程計畫 AI 審查**：讓 AI 照課綱標準幫老師看課程計畫

#104 給 AI「即時查得到」的官方資料，#105 給 AI「本土夠深」的語料底子——一個查、一個學，台灣味才補得齊。

## 配對工具推薦

- [#104 台灣官方資料庫 Twinkle Hub](/tool/104) — 讓 AI 即時查台灣官方資料，跟 #105 是「查得到 × 學得深」雙生
- [#88 課程計畫 AI 審查工具](/tool/88) — 上傳 PDF 依教育局標準逐項自動比對，課綱備課好搭檔
- [#105 臺灣主權 AI 訓練語料庫 × 石門國小](/tool/105) — 本篇主角

## 適用對象

- **本土語文老師** — 一鍵生台語／客語學習單，終於有夠深的本土語素材
- **海洋／在地文化課老師** — 九大主題探索器，輸入單元就撈到對應語料
- **教媒體素養與資訊素養的老師** — 「資料要追到政府源頭」的活教材
- **家長** — 親子共讀、交安媒體素養延伸，每份素材都追得到來源
- **關心臺灣主權 AI 的人** — 看國家級語料如何真正落地到一間國小教室

## 想試試？

→ [前往 #105 臺灣主權 AI 訓練語料庫 × 石門國小](/tool/105)

**先逛逛**：打開九大主題探索器，搜搜「海洋」「本土語」，感受一下 4,554 筆語料的厚度。

**再動手**：填入自備的免費 Gemini 金鑰，讓它幫你生一份台語版學習單——你會發現，**原來 AI 也可以這麼懂台灣**。

國家花十年蒐集的本土語料，這一次，真的走進了國小教室。從石門出發，從海洋開始。
`,
};

const POST_110: BlogPost = {
  slug: 'photo-to-video-110-ai-event-recap-generator',
  title: '#110 活動辦完，照片就躺在資料夾裡發霉？上傳照片，AI 幫你剪成一支有旁白有配樂的成果影片',
  excerpt:
    '運動會、校外教學、創意實作課辦得精彩，幾十張照片拍回來卻只能丟成無聲的相片輪播——因為剪影片要想旁白、要配音、要上字幕、要選轉場，光想就累。#110 讓你只要上傳照片，多模態 AI 自動看懂每張照片在做什麼、編寫前後連貫的繁體中文旁白與字幕、配上微軟 Neural 人聲與背景音樂，一鍵錄製匯出相容 iPhone 的 MP4。純前端、金鑰只存你的瀏覽器，照片不外傳。',
  publishedAt: '2026-06-17',
  readingMinutes: 6,
  tags: ['照片轉影片', '活動紀錄影片', 'AI 自動剪輯', '多模態視覺辨識', 'Neural 語音'],
  toolIds: [110, 68, 94],
  coverEmoji: '🎬',
  coverColor: 'purple',
  body: `## 那個躺在資料夾裡發霉的活動相簿

每次活動辦完都一樣。運動會、校外教學、創意實作課、成果發表——現場辦得多精彩，照片就拍了多少張。回到辦公室，幾十張照片乖乖躺在一個叫「20260617_活動」的資料夾裡，然後……就沒有然後了。

因為要把它剪成一支「像樣」的成果影片，門檻高得嚇人：要想一段通順的旁白、要找人配音（或自己尷尬地錄）、要一張張上字幕、要選轉場別讓畫面硬切、還要配個不踩版權的背景音樂，最後還得會用剪輯軟體把這些兜起來。多數老師忙完一場活動已經沒力氣了，最後只能把照片丟進相簿輪播、配首音樂交差——可惜了現場那些發光的瞬間。

所以我做了 #110：[智慧自動化照片成果轉影片產生器](https://cagoooo.github.io/PhotoLibrary/)。它要解決的就是這一段——**把「一疊照片」到「一支成片」之間，那段最累人的距離，交給 AI 走完。**

## #110 怎麼運作？上傳照片，喝杯咖啡，下載成片

整個流程被我壓成三步，老師不用懂任何剪輯：

**第一步：上傳活動照片** — 拖放或點選多張照片進來，可以拖曳卡片重新排序，排出你要的時間軸。圖片太大會自動壓縮加快分析，建議一次 3～10 張最順。

**第二步：AI 看懂照片，自己編劇** — 這是整個工具的靈魂。多模態視覺辨識會**一張一張看懂照片裡發生了什麼**：場景、人物動作、表情、手上的東西、前因後果，然後像專業導演一樣，編寫一段**前後連貫、積極溫暖、口語化的繁體中文旁白與字幕**，還會幫整支影片擬好主標題與副標題。

它不會寫成死板的「這是一個戴帽子的人在看電腦」，而是寫成「太棒了！我們開始在電腦上編寫屬於我們的第一個程式！」——讓整支影片像在說一個完整的故事，而不是逐張念物品。

**第三步：一鍵產出影片** — AI 配上微軟 Neural 高品質人聲旁白、推薦合適的轉場效果與背景音樂風格，預覽滿意後實時錄製，匯出一支相容 iPhone/Safari 的 MP4。

## 幾個讓成片真的「能見人」的設計

魔鬼藏在細節裡，這支工具讓人驚喜的，是那些「真的有想到」的地方：

- **多種台灣 / 中文人聲** — 走微軟 Edge Neural TTS，曉曉、曉伊、曉佳、臺灣女聲 / 男聲、普通話、粵語多款自然人聲任選，質感接近真人，不是那種機械合成腔。
- **豐富的轉場庫 + Ken Burns 動效** — 淡入淡出、推入、平移、擦除、縮放、模糊過渡、圓形收縮、漸變黑屏……而且 AI 會**依照片內容與故事節奏推薦**該用哪種，靜態照片也能有電影感的緩慢推拉。
- **AI 推薦背景音樂** — 木吉他、溫柔鋼琴、科技律動、復古電子、歡樂滑稽等氛圍，依活動主題自動配；旁白音量還能微調，不怕配樂蓋過人聲。
- **腳本可以手改** — AI 不是黑箱。主副標題、每一幕的旁白與字幕都能逐張編輯再播放預覽，AI 出草稿、你下判斷。
- **匯出相容性 100%** — 可選標準 / 高清 / 超高清，自動轉成 Apple 相容 MP4，傳到 LINE 群組、貼到班網、上傳 YouTube 都不卡格式。

## 真實技術棧：你的照片，不會離開你的瀏覽器

這支工具有一個我很在意的設計原則——**隱私自主**：

- **前端**：純靜態 HTML/CSS/JS + GitHub Pages 零成本部署
- **看照片、寫腳本**：**Gemini 多模態視覺模型**（需自備一把免費 API 金鑰，金鑰只存在你自己的瀏覽器 localStorage，絕不上傳）
- **旁白配音**：**微軟 Edge Neural TTS** 合成自然人聲
- **影片合成**：Canvas + MediaRecorder 在你的瀏覽器裡實時錄製、再轉封裝成相容 MP4

也就是說，整個過程是純前端跑的——你的活動照片**不會被上傳到任何第三方伺服器**。對於拍了很多學生臉孔的校園活動照片，這一點特別重要。

## 跟其他工具的關係：照片→影片這條創作線

阿凱工具集裡，有一整條「**把素材變成作品**」的線，#110 正好補上中間最難的那一段：

- **#68 手作課程照片影片作品上傳平台**：活動現場的照片從哪來？學生作品、課堂花絮先在這裡收齊
- **#110 智慧自動化照片成果轉影片產生器**：把收齊的照片，自動剪成有旁白有配樂的成果影片
- **#94 封面串故事 · 音樂影片分鏡產生器**：想再進一階自己掌鏡分鏡，就接這支

它們的共同信念是：**老師的創意該花在「辦一場好活動」，而不是「事後跟剪輯軟體搏鬥」。**

## 配對工具推薦

- [#68 手作課程照片影片作品上傳平台](/tool/68) — 照片的上游，活動花絮先收齊再餵給 #110
- [#94 封面串故事 · 音樂影片分鏡產生器](/tool/94) — 想自己掌鏡分鏡的進階款，同一條影片創作線
- [#110 智慧自動化照片成果轉影片產生器](/tool/110) — 本篇主角

## 適用對象

- **每學期都要交活動成果影片的老師** — 運動會、校外教學、成果發表，今年起不用再硬剪
- **社團 / 班級經營者** — 想給家長一支有溫度的回顧片，但不會也沒空學剪輯
- **行政承辦** — 校慶、研習、典禮的紀錄影片，上傳照片就有
- **想體驗「多模態 AI 看圖說故事」的人** — 看一個真實情境裡，AI 怎麼把一疊照片讀成一個故事

## 想試試？

→ [前往 #110 智慧自動化照片成果轉影片產生器](https://cagoooo.github.io/PhotoLibrary/)

**先準備**：去申請一把免費的 Gemini API 金鑰（網站右上角有「申請免費金鑰」直達連結），貼進去、測試連線，金鑰就只存在你的瀏覽器裡。

**再試試**：挑 5～8 張你上次活動最有畫面的照片丟進去，看 AI 怎麼幫每一張寫旁白。你會發現——那個躺在資料夾裡發霉的相簿，其實只差一次上傳，就能變成一支讓人想分享的成果影片。
`,
};
...
const POST_118: BlogPost = {
  ...
  excerpt:
    '網路安全、程式思維、數位公民，講完不等於記住。#118 把資訊素養題目放進駕駛艙對戰：選職能角色、答題發動技能、即時看解析；能單挑 AI、同桌雙人，也能投影成全班的觀念討論。',
  publishedAt: '2026-07-13',
  readingMinutes: 6,
  tags: ['資訊教育', '網路安全', '程式思維', '數位公民', '遊戲化學習'],
  toolIds: [118, 108, 11],
  coverEmoji: '⚡',
  coverColor: 'blue',
  body: `## 資訊課最難的，不是把題目講完

資訊課裡有許多「看起來懂了、下次又會忘」的觀念：陌生連結不能隨便點、密碼不能只用生日、假訊息要先找來源、AI 產出的內容也要查證；寫程式時，迴圈和條件判斷不是背名詞，而是解決問題的工具。

這些內容如果只用講義或小測驗，老師通常很快就會遇到兩件事：答得快的孩子覺得太簡單，還在想的孩子則把答題當成被點名的壓力。更可惜的是，選項按完，大家往往只記得「我剛剛對不對」，沒有停下來想「為什麼」。

[#118 石門國小資訊科技駕駛艙：答題快打對戰遊戲](https://cagoooo.github.io/smes-it-quiz-battle/) 想把這段複習，換成一場每個人都願意投入的資訊任務：**用正確的資訊觀念出招，守護校園網路。**

## #118 怎麼解？先選一個資訊職能，再開始作戰

進入遊戲後，學生不是立刻看到題目，而是先選擇自己的資訊職能角色。程式勇者、網安守護者、資料魔法師、創意設計師、AI 探險家、綠能工程師、機器人操控師與雲端航海家，各自有不同的技能語言。這個小小的選擇，會讓孩子從「我要寫題目」轉換成「我要帶著角色完成任務」。

**單挑 AI 或同桌雙人** — 單人模式由駕駛艙 AI 操控防火牆怪、釣魚海妖、漏洞幻影等對手；同桌雙人則讓兩位學生輪流出題、輪流答題。前者適合個人練習與下課挑戰，後者很適合一台平板、兩個人一起討論答案。

**四種主題，對應資訊課的核心觀念** — 可選綜合挑戰、網路安全、程式思維與數位公民。從釣魚連結、強密碼、兩步驟驗證、網路霸凌，到迴圈、除錯、問題分解、AI 使用責任與資訊查證，題目不只考「記得嗎」，而是放回孩子真的會遇到的情境。

**答題就是技能出招** — 基礎、進階、戰術與必殺技能各自帶有不同的題目節奏。答對會造成傷害、累積能量與連擊；答錯不會只跳出叉叉，而是立刻標出正解並說明原因。遊戲保留了競賽感，但把真正的學習點留在每一次回饋裡。

**難度可以跟著班級狀態走** — 練習模式給提示、AI 較慢，適合第一次操作或需要多一點思考時間的孩子；標準模式適合一般課堂；挑戰模式則加快 AI 節奏，讓已經熟悉觀念的學生有更高的目標。

## 我會怎麼帶一節課

如果是四到六年級的資訊課，我會先投影首頁，讓全班一起選「網路安全」和練習模式。第一輪不急著比分數：請學生看到題目先和同桌說出自己的理由，再由一位同學代表點選。答完後，我會停在解析畫面，特別問：「如果奇怪連結是班群裡熟悉的同學傳來呢？」把選擇題延伸成真實情境的判斷。

第二輪再改成同桌對戰。兩人輪流控制角色，規則是答題前一定要先說一句判斷依據；這樣不會讓快手學生一路按到底，也讓每位學生都有說出觀念的機會。課程最後可切到「數位公民」的 AI 使用題，請孩子整理今天自己最想帶走的一條網路行動原則。

若要做差異化活動，完成基礎任務的學生可以挑戰 AI 的進階模式；需要支持的學生則留在練習模式，把題目解析讀完，再回頭嘗試一次。遊戲不會取代老師的引導，但它把重複練習的入口變得更有吸引力。

## 真實技術特色：一個不用登入的課堂駕駛艙

- **純前端網頁**：以 HTML、CSS 與 JavaScript 製作，部署於 GitHub Pages，學生不用建立帳號或交出個資。
- **可檢視的題目資料**：題庫明確標註主題、難度、年級範圍、核心能力與常見迷思概念，方便老師對照課程進度。
- **即時戰鬥狀態**：血量、能量、連擊、75 秒任務計時與出招動畫，讓答題結果有立即而可見的回饋。
- **負責任的 AI 素養**：題目把 AI 的內容查核、誠實標示協助、避免輸入可識別個資等觀念放進任務，而不是把 AI 只當成炫技按鈕。
- **PWA 更新提示**：網站支援更新提示與行動裝置使用；老師可收藏連結，下次課堂直接打開。

## 適用對象

- 想把網路安全、數位公民與 AI 素養從「聽過」帶到「會判斷」的國小中高年級學生。
- 需要課堂暖身、單元複習、分組活動或下課前五分鐘挑戰的班級導師。
- 想用一個免登入工具複習程式思維、資訊安全與數位倫理的資訊科任教師。
- 想把搶答氣氛轉成「輪流想理由、一起看解析」的班級。

## 配對工具推薦

- [#108 資安特攻隊：釣魚郵件偵蒐](/tool/108)：把真假郵件的辨識拉進更完整的資安情境，適合接在網安對戰後深入練習。
- [#11 剛好學 Akailao](/tool/11)：若要讓全班用 QR Code 同步加入題目、蒐集即時作答結果，可搭配這個課堂互動平台。
- [#118 石門國小資訊科技駕駛艙：答題快打對戰遊戲](/tool/118)：本篇主角，適合把資訊課觀念轉成角色技能與即時解析。

## 想試試？

→ [前往 #118 石門國小資訊科技駕駛艙：答題快打對戰遊戲](https://cagoooo.github.io/smes-it-quiz-battle/)

下一次要複習資訊素養時，不妨先讓孩子選一個角色。當「查證來源」「保護帳密」「先理解再使用 AI」不只是標準答案，而能成為他在對戰裡成功出招的理由，這些觀念就更有機會留在真正需要的時刻。`,
};
...
const POST_127: BlogPost = {
  slug: 'ai-literacy-127-taoyuan-competition-prep',
  title: '#127 桃園市AI素養爭霸賽：把「要會 AI」拆成學生今天就能練的六個站點',
  excerpt:
    '#127 桃園市AI素養爭霸賽備戰站，把報名、指定版本、資料蒐集、監督式學習、模型測試、規則紅線與比賽日流程放進同一條練習路線。學生知道今天做什麼，指導教師也能看見隊伍卡在哪一站。',
  publishedAt: '2026-09-14',
  readingMinutes: 7,
  tags: ['AI素養', 'AI競賽', 'PAIA', '監督式學習', '國小資訊'],
  toolIds: [127, 81, 118],
  coverEmoji: '🏆',
  coverColor: 'orange',
  body: `## 「要會 AI」不是一句可以拿來練習的指令

比賽前最容易發生的事，不是學生不努力，而是大家把努力放在不同方向：有人先研究表演賽，有人下載了不對的版本，有人急著寫程式，卻還沒有想過要讓 AI 從哪些資料學會判斷。到了練習時間，隊伍看起來很忙，卻很難回答「今天到底完成了哪一件事？」

桃園市 115 年度 AI 素養爭霸賽的準備，還多了一層現實條件：正式賽要在現場從零蒐集資料、訓練模型、測試、整理檔案與上傳，不能把賽前準備好的資料、模型或程式直接帶進去當答案。對學生而言，這不只是學一個遊戲；對教師而言，也不只是帶隊報名。真正要練的是：**看懂問題、留下有代表性的資料、用模型做推論，再根據結果修正下一輪。**

所以我把這個備戰站做成一條可以照著走的路線，而不是一頁把公告全部貼上的資料庫：先把今天要完成的事排好，再把正式賽的 AI 循環拆成六站，最後用規則紅線與出發前清單把容易失分的細節收好。

## 先做完四件事，隊伍才真的開始

網站的第一段先處理最急、也最容易被拖到最後的四個動作：確認兩位學生的隊伍與組別、在官方期限前完成報名、安裝指定版本、從零走過一次練習流程。這個順序很重要，因為如果報名身分還沒確認，或 PAIA Desktop 與正式賽遊戲版本還沒對好，後面花再多時間練習都可能練錯場地。

頁面把正式賽需要核對的內容寫在同一處：PAIA Desktop 3.3.5、魷來魷去對戰版 1.7.5，以及遊戲設定中的地圖 12。它也把老師與學生的責任分開：報名和身分資料由老師、學校端處理；學生則先熟悉遊戲、輪流觀察資料與操作測試。日期和規則可能因官方公告更新，備戰站特別提醒要回到桃園市官方入口核對，這是行前工具應該保留的誠實界線。

## 六站練習，讓「模型為什麼失分」說得出來

正式練習不是一直玩到分數變高，而是每一輪都走過六站：

1. **看懂遊戲**：辨認食物、垃圾、對手、座標、角度與距離，先說出希望 AI 學會什麼。
2. **蒐集資料**：留下「當時看見什麼、最後做了什麼」的樣本，而且要有不同距離、角度與場上狀況。
3. **訓練模型**：使用正式賽要求的監督式學習，理解模型是在樣本中找規律，不是把 if／else 規則換一個名字。
4. **測試推論**：用模型沒看過的情境驗證，觀察 AI 是否真的能判斷，而不是只記住練習題。
5. **迭代修正**：一次只改一件事，補資料、調整特徵或重新訓練，再比較結果。
6. **匯出上傳**：確認 ml_play.py、模型檔、檔名、容量與線上檢測，每一項都能在比賽前說明白。

這樣的拆法對國小生特別有幫助。當模型撞到垃圾時，老師不必直接給答案，可以回問：「這個角度的資料有沒有收過？」「你這次改的是資料、模型還是程式？」「如果只看一次高分，能不能證明模型穩定？」學生開始把失分看成可以檢查的證據，而不是「AI 今天運氣不好」。

## 比賽規則要變成練習的一部分

備戰站另外整理「規則紅線」：不能直接把現成資料、模型或程式帶進場，不能用硬編碼規則代替模型決策，檔案要符合指定格式與 20 MB 總量限制，上傳後也要確認 AI 在競賽平台能正常執行。這些內容如果只在比賽前一天提醒，通常已經太晚；最好的做法是在每次模擬時就加入一個「從零開始」的檢查。

我會把兩位學生安排成資料觀察員與程式測試員，每一回合交換角色。前者負責描述場上看見的情境，後者負責拖拉積木、訓練、測試與記錄；交換之後，兩個人都要能獨立完成流程。網站的 READY CHECK 也把學生卡、隊伍資料、指定版本、流程口訣、檔案命名與報到交通列出來，讓教師可以在出發前逐項確認，而不是靠記憶問「還有沒有漏帶什麼」。

## 我會怎麼帶一節練習

第一輪先不追分。讓全班看一局完整流程，請學生說明 AI 看到什麼、資料如何留下、模型如何產生動作。第二輪開始計時，但只要求完成蒐集、訓練、測試與保存。第三輪再比較兩個候選模型，要求隊伍寫下「這次只改了什麼」和「下一輪要補哪一種情境」。到了賽前，再用兩小時從零開始的模擬，練習檔案核對、上傳檢測和兩人的口頭分工。

這套節奏也適合不同程度的學生。剛入門的孩子先練習看懂物件與描述資料，熟悉的孩子再討論樣本代表性、模型穩定度和測試公平性。教師不需要另外做一份大型講義，打開備戰站就能從目前的航段開始；學生回家也能用同一個網址回看規則與官方資源。

## 技術細節也服務了現場

這不是把 PDF 截圖放上網的靜態公告，而是響應式 GitHub Pages 網站：導覽列可跳到六大段落，報名截止倒數會提示時間，快速測驗用四題檢查關鍵概念，READY CHECK 的勾選狀態保存在瀏覽器本機。頁面同時放入桃園市官方入口、教育部遊戲介紹、教材、PAIA 登入、下載與影片課程，降低教師在不同群組和分頁之間找資料的成本。

當然，互動介面不等於官方規則本身。網站清楚把自己的教練建議和競賽實施計畫、全國細則分開，日期、版本、場地與限制仍以主辦單位最新公告為準。這也是我希望學生在 AI 素養競賽中練到的能力：會使用工具，也會查證工具說的話從哪裡來。

## 配對工具推薦

- [#81 國小資訊科技教學駕駛艙入口網](https://cagoooo.github.io/it-cockpit/)：把資訊科技課的簡報、影片、互動關卡與評量集中在一個入口，適合教師先建立課程脈絡，再把 AI 競賽備戰站放進六年級資訊或彈性課程。
- [#118 石門國小資訊科技駕駛艙：答題快打對戰遊戲](https://cagoooo.github.io/smes-it-quiz-battle/)：用遊戲化答題複習網路安全、程式思維、數位公民與 AI 素養；先練概念，再進入 #127 的資料、模型與競賽流程，學生比較容易知道自己為什麼要做這些準備。

## 適用對象

- **國小參賽學生**：依六站流程練習，並用測驗與出發前清單檢查自己是否真的準備好。
- **指導教師與資訊科任**：安排隊伍分工、模擬時間、資料紀錄與模型測試，不必重新整理散落的規則。
- **班級導師**：協助學生確認報名、身分證明、合作方式與比賽日行程。
- **教務、學務與資訊承辦**：集中轉發報名入口、指定版本、檔案限制與官方資源，讓不同單位使用同一份行前資訊。

→ [前往 #127 桃園市AI素養爭霸賽備戰站](https://cagoooo.github.io/ai115-shimen-prep/)

如果隊伍下一次練習不知道該做什麼，先不要急著換模型。打開備戰站，從目前所在的那一站開始，完成一次「資料 → 訓練 → 測試 → 修正」循環；等你能說出這一輪改了哪一件事，分數通常才真正有機會往上走。`,
};

const POST_126: BlogPost = {
  slug: 'deadzone-126-3d-tower-defense-energy-weapons',
  title: '#126 DEADZONE 能量防線：把失敗放在策略上，孩子才會想再試一次',
  excerpt:
    '#126 是一款直接在瀏覽器跑的 3D 塔防射擊：三種能量武器隨時切換、三種防禦塔各有分工、十波殭屍撲向能源核心。最關鍵的設計是角色不受傷害、敵人只攻核心——失敗回饋落在「這一波該把塔蓋在哪裡」，是能被討論、被修正、被下一局驗證的判斷，孩子因此願意再想一次。',
  publishedAt: '2026-09-06',
  readingMinutes: 6,
  tags: ['3D塔防', '策略思考', '資源取捨', 'Three.js', '資訊課素材'],
  toolIds: [126, 120, 102],
  coverEmoji: '🎮',
  coverColor: 'green',
  body: `## 一個很現實的觀察：孩子放棄的時間點

我在資訊課放過不少小遊戲，觀察到一個很一致的現象：**讓孩子放棄的通常不是「太難」，而是「重來的代價太高」。**

碰到敵人就立刻結束、一結束就得從頭再來、前面五分鐘的布局全部歸零——這種設計在遊戲市場上很常見，因為玩家是自己選擇要挑戰的。但放進四十分鐘的課堂，它會造成很現實的兩極化：反應快的孩子越玩越順，手速慢的孩子重來幾次之後就把視窗關掉，開始找別的事做。

更關鍵的是，這種設計把失敗的原因全部指向「我手不夠快」，而那跟我真正想讓孩子練的東西——觀察、判斷、規劃、取捨——幾乎沒有關係。**教育上真正有價值的失敗，是那種可以被說出原因、可以被修正、下一次能驗證的失敗。**

所以在做 **DEADZONE｜能量防線** 的時候，第一條定下來的規則不是武器有幾種，而是：**玩家角色不受傷害，敵人只攻擊能源核心。**

## 這個規則，改變了孩子面對失敗的方式

角色不受傷害，代表孩子可以放心走進戰場中間、繞到彎道後面、試試看站在哪個角度射得到最多敵人。他不會因為「站錯位置」被立刻懲罰，只會因為「塔蓋錯位置」而讓核心掉血。**嘗試的成本被壓到很低，探索才會真的發生。**

於是孩子講得出口的失敗原因，從「我反應不夠快」變成「我這一波的火力沒有覆蓋到那個彎道」。這是完全不同層次的兩句話——後者可以被討論、被修正、被下一局驗證，也才有辦法在課堂上接著問「那你下一局打算怎麼調整？」

遊戲的結束畫面也刻意不只寫一個 GAME OVER，而是告訴你撐到第幾波、擊退幾隻，並直接給一句可以照著試的戰術建議：試試在彎道搭配冰凍稜鏡與電漿迫擊塔。失敗在這裡不是句點，是下一輪的提示。

## 三種能量武器：熱量條逼你做選擇

玩家手上有三把可以隨時切換的能量武器（鍵盤 1／2／3，手機點武器卡）：

- **脈衝步槍**：傷害 23、射速最快的通用型，射程也最遠。打雜兵很順。
- **電漿重砲**：單發傷害 64，附帶爆炸範圍，但冷卻長、發熱高。適合一群敵人擠在彎道時一發清掉。
- **冰霜射線**：單發只有 10 點傷害，但它的價值不在傷害，而在**減速**。拿它把快速殭屍拖慢，你的塔就多了幾秒輸出時間。

真正讓武器切換變成「決策」而不是「裝飾」的，是三把槍**共用同一條熱量條**。電漿重砲一發就吃掉 28 點熱量，你不可能整局壓著它掃射；熱量滿了會強制冷卻，那幾秒你只能看著敵人往前走。所以孩子會很自然地學到一件事：**火力要分配，不是一直用最強的那把。**
...
| 塔 | 造價 | 角色 |
|---|---|---|
| 脈衝哨塔 | 100 | 穩定連射，單體輸出的基本盤 |
| 冰凍稜鏡 | 125 | 幾乎不造成傷害，專門減速控場 |
| 電漿迫擊塔 | 175 | 範圍爆破，一次打一群 |

開局只有 300 點能源。這個數字是刻意的——它剛好不夠讓你同時買一座迫擊塔和一座稜鏡，所以第一個決定就必須取捨。塔可以升級，也可以出售並返還已投入能源的 70%，代表「押錯位置」不是死局，但也不是沒有代價。

地圖是一條帶了好幾個直角彎的路徑。遊戲的戰術提示直接把答案講白了：**彎道是最好的伏擊點**。原因很單純——敵人在彎道會走比較久的路徑通過同一個塔的射程圈，等於免費多打好幾秒。這個道理不用我在黑板上畫圖，孩子自己蓋錯兩次就懂了。

敵人也有節奏：普通殭屍打底，第 2 波起混入快速殭屍，第 3 波起出現重型殭屍，第 5 波和第 10 波各有一隻血量 1000 的巨型殭屍。撐過十波就贏。一局大約十分鐘，剛好是一節課的一個段落，或一次下課。

## 真實技術特色：一款要能在學校電腦上跑起來的 3D 遊戲

戰場是用 Three.js 建的，殭屍、守衛與貨櫃、水塔、油桶等場景物件都是 GLTF／GLB 模型，用 Vite 打包後部署在 GitHub Pages。音效比較特別——它不是音檔，而是用 Web Audio 即時合成的音調，所以整包載入量比想像中小很多。

不過真正花時間的不是 3D，是**讓它在學校的電腦教室裡不出事**：

- **沒有 WebGL 2 就明講**。學校電腦硬體加速被關掉是很常見的事。這時候不會給一片黑畫面，而是直接寫出「請使用支援 WebGL 2 的 Chrome 或 Edge，並開啟硬體加速後重新載入」。
- **準備階段自動存檔**。波次、能源與塔的配置會存進 localStorage，孩子不小心重新整理，會回到最近一次的部署階段，而不是從頭開始。
- **改版後的舊快取會自動修好**。這是 PWA 最惱人的坑：新版上線但瀏覽器還抓著舊的程式碼片段，畫面直接白掉。這裡加了一層攔截，偵測到載入失敗會自動註銷 Service Worker、清掉快取再重載，並在畫面上說明「正在同步遊戲程式」，而不是讓孩子看著白畫面舉手叫老師。
- **桌機和手機是兩套操作**。桌機是 WASD 移動加滑鼠瞄準、Shift 奔跑；手機自動切成左右雙搖桿，左手移動右手瞄準開火。平板加到主畫面之後可以離線玩。

另外還有五步驟的新手訓練提示條、暫停與倍速、全圖視角、瞄準輔助與音效開關——都是為了讓「第一次打開的人不用問人就能開始」。

## 我會怎麼帶

**當成課間或獎勵活動**：一局十分鐘剛剛好。因為角色不受傷害，程度落差大的班級不會有人卡在開頭就放棄，願意動腦的孩子則會去挑戰第 5、10 波的巨型殭屍。

**當成資訊課的策略討論**：我會先讓全班在電子白板上一起玩一局，每次要蓋塔就停下來問「蓋哪裡？為什麼？」，然後把塔的造價、傷害、射程三個數字寫在旁邊，讓他們自己算「175 塊的迫擊塔，值不值得等於一座哨塔加一點錢」。這是很具體的數值取捨練習。

**當成 3D 網頁教材**：高年級或國中的資訊課，可以直接開開發者工具對照——Three.js 場景怎麼組、GLTF 模型怎麼載、WebGL 偵測失敗怎麼降級、PWA 怎麼離線。它是一個活的、可以按 F12 拆開來看的範例，比投影片上的程式碼截圖有感覺得多。

## 配對工具推薦

- [#120 3D 星際雷霆：隕石防禦與太空探索解題大冒險](https://cagoooo.github.io/space-meteor-evasion-3d/)：同樣是 3D，但鏡頭是第一人稱駕駛艙。兩款放在一起，正好可以讓學生比較「第一人稱」與「等距俯視」在空間判斷上的差別。
- [#102 外星人入侵·保衛石門 — 復古太空射擊 × 全校排行榜](https://cagoooo.github.io/alien-invasion/)：同樣是「守住基地」的主題，但是 2D Canvas。適合對照 2D 與 3D 在效能、載入與操作設計上的取捨。
- [#126 DEADZONE 能量防線：3D 塔防射擊](https://cagoooo.github.io/zombie/)：本篇主角。

## 適用對象

- **導師**：需要一個十分鐘結束、不用註冊、不會有人一直卡關的課間活動。
- **資訊科任教師**：想要一份可以按 F12 拆開來講的 3D／WebGL／PWA 現成教材。
- **數學與綜合課教師**：可以用塔的造價、傷害、射程做很具體的取捨與比較練習。
- **資訊組長與活動承辦**：需要一款免帳號、無廣告、不上傳任何資料、全程在瀏覽器本機執行的校內遊戲素材。
- **學生**：喜歡動腦更勝手速的孩子，這款會比純反應型遊戲耐玩。

## 想試試？

→ [前往 #126 DEADZONE 能量防線：3D 塔防射擊](https://cagoooo.github.io/zombie/)

第一局先別急著蓋塔。花三十秒把地圖看一遍，找出那幾個直角彎——你會發現整局遊戲的答案，其實在敵人還沒出現之前就已經決定了。

`,
};

const POST_125: BlogPost = {
  slug: 'teacher-union-125-membership-service-hub',
  title: '#125 教師工會支會宣導站：把「今年會費到底怎麼繳」一次講完，不用再翻群組',
  excerpt:
    '#125 桃園市教育產業工會石門國小支會的會員服務暨活動宣導站，把會費、續會禮、新進入會優惠與親子健行活動收進一頁。分「我是舊會員／我是新進老師」兩條入口，寫明本校採教師小組代收、財務長統一匯款，會員不必個別匯款，也不必自己去填官方表單。',
  publishedAt: '2026-09-05',
  readingMinutes: 5,
  tags: ['教師工會', '會員服務', '活動宣導', '行政溝通', '校內公告'],
  toolIds: [125, 62, 74],
  coverEmoji: '👥',
  coverColor: 'blue',
  body: `## 每年都要重講一次的那件事

學校裡有一類事情，資訊其實從來沒有短缺過，但每年都會被重新問一遍：**工會會費今年多少？我要自己匯款嗎？匯到哪個帳號？我是新來的老師，要找誰？**

問題出在資訊的形狀。工會的正式公告是一份公文，續會優惠寫在另一張海報，活動報名連結貼在群組某一則已經被洗掉的訊息裡，而「我們學校是怎麼辦的」——這條最關鍵的資訊——往往只存在支會長和財務長的腦袋裡。於是每到十月，同一段對話會在不同的訊息視窗裡重播十幾次。

更麻煩的是，資訊分散不只是浪費時間，還會出錯：有人看到官方表單就自己填了、有人看到帳號就自己匯了款，結果財務長要花更多時間去對帳、去請人退款。**在會務這件事上，「講得不夠清楚」的成本，是後面要花三倍時間收拾。**

## #125 怎麼處理：先分流，再說細節

這個宣導站沒有做成一份長長的公告，而是先問你一個問題：**你是舊會員，還是新進老師？**

首屏兩個按鈕就把人分成兩條路。因為這兩種人要做的事完全不同——舊會員關心的是「什麼時候交、能拿多少續會禮」，新進老師關心的是「我要找誰、有沒有免入會費」。把兩條路分開之後，每個人要讀的內容都少了一半。

**辦理方式寫成三步驟。** 先向本校登記（舊會員交給今年的教師小組成員，新進老師直接找支會長）→ 支會長彙整資料與款項 → 財務長一次匯款並與工會對帳。這三步是這頁最重要的內容，因為它回答的是「我在這條流程裡負責哪一段」。

**帳號寫出來，但同時寫清楚誰不該用它。** 頁面上有財務長統一匯款的郵局帳號，但緊接著就標明「僅供財務長統一匯款使用，一般會員請勿個別匯款」。這是一個很小、但很關鍵的設計：資訊公開，同時把誤用的路堵起來。

**優惠寫成有期限的階段，不寫成一句「請儘速繳交」。** 舊會員的續會禮分兩階段，早一點完成回饋較高；新進與中斷會員在期限內預繳可以免收入會費，還加贈當年度會籍與會員卡。把回饋和時間綁在一起，比催繳有效得多。

**會員身分不含糊。** 代課、代理教師屬於普通會員，有選舉與被選舉權；非教學工作者是贊助會員，沒有選舉權但其餘權益相同。這段話寫出來，新進的代理老師就不必私下問「我這樣算不算會員」。

## 活動宣導：把「報名前想知道的事」全部前置

站上的另一半是親子健行活動。這一段我覺得特別值得參考，因為它把活動宣導頁該有的欄位補得很齊：

- **報名門檻寫在前面**：會員專屬、名額有上限、每名會員可攜眷的人數也寫明，避免報了名才發現不符資格。
- **當日流程切成四段**：報到、健行集章、兌換伴手禮、賦歸，每段都標了時間區間，家長可以直接安排當天行程。
- **研習時數怎麼登錄有寫**：全程參與可核發 3 小時，並說明要在活動當日依教育發展資源入口網的「活動查詢」流程登錄。這句話讓教務處少回答二十封信。
- **請假與缺席規範寫清楚**：無法參加要在期限前寄信，未請假缺席會影響後續參加活動的資格。醜話說在前面，遞補才跑得動。
- **風雨的處理方式也寫了**：活動照常舉行，若因不可抗力延期或取消會在官網與粉專公告，恕不個別通知。

這一整段其實可以直接當成「校內活動宣導頁的欄位檢查表」——不管你辦的是健行、親職日還是校慶。

## 真實技術特色：把會務資料抽出來，明年只改一個檔

這是一個純靜態網站，HTML、CSS、JavaScript 部署在 GitHub Pages，不需要後端，也不需要登入。但它做了一件讓明年的自己會感謝今年的事：**會務內容抽到獨立的資料檔集中維護**，年度、金額、日期、活動資訊都在同一個地方，明年換屆換年度時只要改資料，不用去動版面程式碼。

另外它註冊了 Service Worker，並在資源網址掛上版本查詢字串。實際效果是：同仁可以把這頁加到手機主畫面當成獨立 App，離線也能查會費與流程；而網站改版之後，不會卡在舊快取裡看到過期的日期。

最後是 Open Graph 與 Twitter Card 標記做得完整，含 1200×630 的預覽圖。這點在校園情境裡比想像中重要——因為大部分同仁不是從瀏覽器打開它，而是從 LINE 群組的那則貼文點進來的。連結貼出去長什麼樣子，決定了有多少人會點。

## 我會怎麼用它

如果我是支會長或行政承辦，我會把這頁當成**唯一的對外答案**：群組裡有人問會費，就貼連結；有人問活動報名，還是貼同一條連結。不是因為懶得回答，而是因為口頭回答會產生版本差異——第三個人問的時候，答案可能已經跟第一個人不一樣了。

如果我是教師小組成員，我會在收費前先把「三步驟辦理方式」那段截圖，貼在辦公室的群組裡，讓大家知道錢交給誰、後面是誰在處理。

如果我是新進老師，我只要看首屏右邊那張「本校辦理方式」的小卡：舊會員小組成員代收、新進老師支會長協助入會、匯款由財務長統一處理。三行看完就知道自己要找誰。

## 配對工具推薦

- [#62 桃園市龍潭區石門國民小學114學年度親職教育日](https://cagoooo.github.io/114_Parent/)：同樣是「一頁講完一件校園活動」的宣導站，可以對照兩者在流程、時程與 FAQ 上的寫法。
- [#74 2026 親職日場地配置圖](https://sites.google.com/mail2.smes.tyc.edu.tw/academic/2026%E8%A6%AA%E8%81%B7%E6%97%A5%E5%A0%B4%E5%9C%B0%E9%85%8D%E7%BD%AE)：活動當天的空間資訊補位，宣導頁講「怎麼報名」，配置圖講「到了要往哪走」。
- [#125 桃園市教育產業工會｜石門國小支會會員服務暨活動宣導](https://cagoooo.github.io/TeacherGroup2026/)：本篇主角。

## 適用對象

- **支會長、財務長與教師小組成員**：需要一份可以直接轉貼、不會被口頭轉述改壞的標準答案。
- **各處室行政同仁**：辦理研習、活動或收費業務時，可以照這頁的欄位檢查自己的公告有沒有漏。
- **導師與科任教師**：想確認自己今年要不要續會、能不能帶家人參加活動，一頁看完就能決定。
- **新進與代理教師**：搞不清楚自己算哪一種會員、該找誰辦理時，這是最省時間的入口。

## 想試試？

→ [前往 #125 桃園市教育產業工會｜石門國小支會會員服務暨活動宣導](https://cagoooo.github.io/TeacherGroup2026/)

如果你的學校也有一件「每年都要重講一次」的事——不管是工會會務、社團招生還是志工招募——這頁的結構可以整套借走：先分流，再講本校怎麼辦，最後把常見問題寫在下面。做一次，之後每年只要改資料。
...
const POST_123: BlogPost = {
  ...
  excerpt:
    '#123 校網無障礙 AA 遷移操作平台，把「舊校網搬到教育局共構平台、拿到無障礙 AA 標章」這件沒人說得清楚的事，拆成 12 個由上往下、做完打勾就好的步驟：要貼的內容一鍵複製、進度自動記住，還內建對比度小工具與會讓你秒退件的四大雷區清單。',
  publishedAt: '2026-08-06',
  readingMinutes: 6,
  tags: ['校網遷移', '無障礙AA', '網站標章', '資訊組長', '行政減負'],
  toolIds: [123, 107, 82],
  coverEmoji: '🎛️',
  coverColor: 'green',
  body: `## 「校網要搬家，順便拿無障礙標章」——然後呢？

如果你是學校的資訊組長或網管，這句話你大概聽過。聽完之後你會拿到：一本研習講義、一份無障礙規範 PDF、一個教育局共構平台的後台帳號，還有一個死線。

然後你會發現一個很現實的問題：**這三份資料沒有任何一份告訴你「先做哪一件」。**

- 講義是按功能分章的，不是按你該做的順序寫的。
- 規範寫的是「應符合對比度 4.5:1」，不是「你們學校的處室選單底色 #7e8f62 對比只有 3.50，改成 #5d6b47 就過了」。
- 後台可以改的地方有上百處，但真正會害你退件的只有那幾處。

更慘的是，這件事不是一個晚上做得完的。你今天改了三個頁面，隔週回來已經忘記做到哪、哪些改過、哪些還沒。等到要送審那天，只好從頭再掃一遍。

**「有沒有一個東西，就照著它從上到下做，做完打勾，關掉明天再開還記得我做到哪？」**

這就是 **[#123 校網無障礙 AA 遷移操作平台](/tool/123)** 的全部目的。

## #123 怎麼解？把「知識」壓成「動作」

**功能 A：12 個步驟、三個階段、每步都標時間**
分成「送審前 8 步」「送審 3 步」「送審後 1 步」，每張卡片右上角標了預估時間——從「約 1 分」到「約 60 分」。你可以看著今天剩下的空堂，決定要吃掉哪幾步。分階段本身也是一種提醒：**送審前只需要做完第一區塊**，因為研習與中心都強調「做越多越容易被挑毛病，趁資料不多時送審」。

**功能 B：進度自動記住**
勾選狀態存在瀏覽器本機，關掉重開、隔天再打開、下週再回來，進度條都還在。頂部同時顯示完成度（0 / 12）與距離標章死線的天數，不用自己算。

**功能 C：要貼的內容，一鍵複製整份**
5 個首頁區塊的修正原始碼、要改的色碼、要跑的檢查指令，全部做成「複製」按鈕，整份進剪貼簿。這一步修掉的是 9 條從舊站沿用 \`/uploads/\` 路徑、搬過去就會掉到共構平台首頁的死連結，加上一堆用詞不合規的 title——**總共 55 處，靠手工一個一個找是找不完的。**

**功能 D：對比度問題，直接給你答案**
不是告訴你「要 ≥ 4.5」，而是告訴你：處室選單底色 \`#7e8f62\` 對比 3.50 不合格，改成 \`#5d6b47\` 變 5.74，然後**記得把後台右邊那個「複製顏色到所有處室」開關打開**——7 個處室一次套用完成。一分鐘，27 處。

Bootstrap 那組也一樣：\`.text-secondary\` 是 #6c757d，在淺灰底上只有 4.45～4.48，就差那麼一點點。平台直接給你「原本 → 改成」的對照表，以後寫內容避開 \`.text-secondary\` 和 \`.text-muted\`，要淺灰質感就用 \`#495057\`。

**功能 E：內建對比度小工具**
頁尾附一個即時計算器。臨時想確認某組新配色過不過，輸入前景色與背景色就有答案（內文要 ≥ 4.5、大字 ≥ 3），不用另外開別的網站。

**功能 F：送審四大雷區清單——踩到就秒退，還要等 7 天**
這一段是整個平台最值錢的部分，因為這些事沒人會事先告訴你：

- 網址要是 \`https://\`，**結尾不可有 index.php、後面不加斜線**。
- 網站名稱要含縣市（例如「桃園市龍潭區石門國小 全球資訊網」），但**絕不可含「無障礙」或「測試」**。
- 一定要用**機關代碼**送審，用私人機構會直接退回。
- 標章等級選 **AA（中間那個）**，很多人在這步選錯。
- 「網站導覽」四個字**一字未改**，改了直接退。

平台還補了一句很重要的判讀：收到「限期改善」其實是好消息，代表快過了，5 天內改完點「回覆人工檢測報告」就好；收到「退件」才要等 7 天。

**功能 G：Freego 檢測的環境地雷也寫進去了**
JRE 版本必須 17～21（太高太低都不行）、安裝時要把 JavaSoft registry keys 改成「安裝」、chromedriver 要去查 Stable 版本對版下載。還有一句能省你半小時的：掃描時 \`tad_login\` 那頁載入逾時**是正常的，不影響送審**。

## 真實技術棧

- **前端**：純單檔靜態 HTML + CSS + 原生 JavaScript，無框架、無後端、無登入
- **進度保存**：localStorage，勾選即存，關頁不掉
- **對比度計算**：WCAG 相對亮度公式前端即時運算
- **主題**：預設亮色，可手動切深色；未切過時才跟隨系統偏好
- **版面**：桌機用 \`min(1320px, 94vw)\` 容器搭配 \`auto-fill\` 卡片網格，螢幕越寬欄數越多，不會把內容擠成中間一條窄欄
- **部署**：GitHub Pages，含 PWA manifest 與 apple-touch-icon，可加到桌面當獨立 App

有一個設計上的取捨值得說：**含個人信箱與分機的成員批次資料，公開版一律不放。** 頁面只標註本機資料夾位置，需要那份資料的人自己去電腦上開。可用性跟個資保護，這裡選了後者。

## 教學現場情境

**情境一：資訊組長的下午空堂。**
打開平台，看到「進度 3 / 12」，直接跳到第 4 步「改三個頁面的文字顏色（約 10 分）」。做完打勾，關掉。下週再開，進度還在 4 / 12。

**情境二：把工作分出去。**
第 2 步要上傳的 5 個檔案屬學務處、第 6 步的成員資料屬輔導室與幼兒園。清單本身就是分工表——把網址傳給承辦人，讓他們自己照著那張卡片做，連「批次輸入框只有在該處室還沒有任何成員時才會出現，所以先貼批次、不要先手動加人」這種細節都寫好了。

**情境三：送審前一晚的最後檢查。**
第 8 步「跑檢查確認全部歸零」給了三行指令，目標明確寫著：站內連結失效 0 項、對比度 0 組不合格。自己先掃過比等 Freego 掃全站快很多。

## 跟其他工具的關係

阿凱老師的工具集裡，資訊組長這條線已經有 [#107 學生教育雲帳號更新工具](/tool/107) 處理帳號、[#82 教室資訊設備盤點系統](/tool/82) 處理設備。#123 補的是第三塊、也是最難外包的一塊：**校網本身。**

這三個工具的共同設計哲學是一樣的——**不是做一個更漂亮的後台，而是把「只有承辦人腦袋裡才有的順序」寫下來，讓下一個人接手時不用重新踩一次雷。**

## 配對工具推薦

- [#107 學生教育雲帳號更新工具](/tool/107) — 資訊組長的另一條例行流水線
- [#82 教室資訊設備盤點系統](/tool/82) — 設備盤點也是一份「照著做就好」的清單
- [#123 校網無障礙 AA 遷移操作平台](/tool/123) — 本篇主角

## 適用對象

- **資訊組長／學校網管**：正在做校網遷移，或準備申請無障礙標章的第一線執行者。
- **各處室行政承辦人**：教務、學務、總務、輔導都有自己要維護的頁面與要上傳的檔案。
- **已經有標章的學校**：認證有效期 3 年、期間會不定期抽查，平台裡的撰寫紀律（公告不可只放海報圖、掃描件不可直接放、檔名要有意義的中文）是長期要守的。
- **國中、高中**：無障礙規範與送審流程全國一致，步驟同樣適用，只有共構平台的後台細節依縣市略有差異。
...
// 執行期取文章請用 ./postLoader（只載入單篇正文）；此檔保持為撰寫用的唯一資料源。
