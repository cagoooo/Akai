    ...
    <PullToRefresh onRefresh={handleRefresh}>
      <div className="min-h-screen bg-background">
        <main id="main-content" role="main" className="container mx-auto px-3 sm:px-6 py-4 sm:py-6 md:py-8" aria-label="教育工具列表">
          ...
          <div className="flex flex-col xl:flex-row gap-4 sm:gap-6 xl:gap-8">
            ...
            <div className="w-full xl:w-2/3 space-y-5 sm:space-y-8">
              ...
              {/* 最近使用區塊 */}
              {hasRecent && !isLoading && (
                <section className="p-3 sm:p-4 rounded-lg bg-teal-50 mt-4">
                  <div
        ...
        <Button
          variant="outline"
          size="icon"
          className="fixed bottom-20 right-4 sm:right-6 rounded-full shadow-lg hover:shadow-xl transition-all z-40"
      ...
      {/* 手機版滑動選單 (僅包含篩選器，不在桌機顯示) */}
      <MobileSidebarDrawer isOpen={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
        <div className="space-y-5 pb-8">
          <Suspense fallback={<div className="h-32 rounded-xl bg-orange-100/50 animate-pulse" />}>
            <AdvancedSearch
              // 注意這裡不綁定 ref 以免搶走桌機版快捷鍵的 focus
              searchQuery={searchQuery}
              onSearchChange={setSearchQuery}
              resultCount={filteredTools?.length || 0}
              totalCount={toolsData?.length || 0}
            ...
            <div className="mt-4">
              <CategoryFilter
                ...
                onCategoryChange={(val) => {
                  ...
                  setIsSidebarOpen(false); // 點擊分類後自動關閉選單，增加流暢度
                }}
                categoryCounts={categoryCounts}
                showFavorites={showFavorites}
                onToggleFavorites={() => setShowFavorites(!showFavorites)}
