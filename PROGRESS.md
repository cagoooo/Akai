...
## 🎯 當前版本狀態
- **當前版本**: `v3.2.3`
- **最後更新狀態**: 完成「穩定隨機排序 (Stable Random Sort)」，解決互動時卡片跳動問題。

## 📌 完成功能總覽

### `v3.2.3` (最新)
- **卡片互動穩定性優化**
  - 實作「會話級穩定隨機排序」：引入 `sessionSeed` 機制，確保在同一次訪問中，隨機排序的結果在重新渲染時保持不變。
  - 徹底解決點擊「複製連結」跳出 Toast 或開啟「QR Code」視窗時，背景卡片重新洗牌跳動的 UX 痛點。
  - 重構 `Home.tsx` 資料流：移除不必要的重複洗牌邏輯，統一由 `useSortOptions` 提供確定的排序結果。
