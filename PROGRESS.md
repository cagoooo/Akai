...
## 🎯 當前版本狀態
- **當前版本**: `v3.6.108` (本機/CI) · 公開工具總數 **122 個** 🎮🚀
- **里程碑**: **2026-07-29 推薦精靈補上常駐入口** — 在這之前精靈只有「自動彈出」一條路，按過「稍後再說」就再也找不回來；現在 Hero 下方有入口卡、快速跳到列有 🎯 chip。
- **最後更新狀態**: 見下方 `2026-07-29`；前一批 P0-1～P0-5 見 `2026-07-28` 第七批；未來待辦見 `FUTURE_OPTIMIZATION_V3.6.100.md`
...
### `2026-07-29` 🚪 推薦精靈補上常駐入口（使用者在正式站上「找不到選族群的地方」）`v3.6.108`

**回報**：在 MacBook 上看 cagoooo.github.io，整頁找不到可以選族群對象的區塊。

**根因**：`BulletinHome` 只在 `audienceProfile` 存在時渲染身分徽章與「為你推薦」條；
**還沒選過身分的訪客，首頁上沒有任何入口** —— 精靈只有「自動彈出」這一條路。
而 `shouldAutoOpenAudienceWizard()` 遇到下列任一情況就不會自動開：

- 這個瀏覽器 session 按過一次「稍後再說」（`akai_audience_wizard_dismissed_v1`）
- 網址帶 `?q=` / `?category=` / `?tag=` / `?favorites=` / `?wish=` 任一參數

也就是說，只要關過一次，訪客就再也叫不回精靈 —— 這正是使用者遇到的狀況。

**修法**（兩個入口，兩種可見度都顧到）：

| 位置 | 內容 | 顯示條件 |
|---|---|---|
| Hero 正下方 | `AudienceEntryCard` 黃色便利貼：「不知道從哪個工具開始？花 20 秒說說你的身分與需求」 | 只在**還沒選過身分**時顯示 |
| 快速跳到列 | `BulletinQuickNav` 多一顆橘色 🎯「找適合我的工具」動作 chip | 同上（傳 `onFindTools` 才出現） |

已經選過身分的人不會看到這兩個入口 —— 他們上方本來就有身分徽章與「重新設定」，
再多一張卡只是重複。

**驗證**：`tsc`／`eslint --max-warnings 0`／274 項測試全綠（新增 3 項）；
preview 實測「設了 dismissed 旗標 → 精靈不自動開 → 兩個入口都在 → 點任一個都能叫回精靈」。

> 📌 這條再次驗證 memory 裡那條「新內容必補主頁入口（≤1 次點擊原則）」：
> 功能可以運作 ≠ 訪客找得到。這次是「自動彈出過一次」就把入口用掉了。

### `2026-07-28` 第七批（🎯 引導體驗 P0 五連發）`v3.6.107`

接第六批之後，把「引導動線」這條線一次補完。五項全做。

| 編號 | 項目 | 問題 | 做法 |
|---|---|---|---|
| **P0-1** | 作答草稿可接續 | 第六批擋掉了「被自動更新打斷」，但**根本痛點還在**：關掉精靈或任何原因重整，`RESET` 直接清空，回來要從第一題重選 | 新增 `lib/audienceWizardDraft.ts`（sessionStorage，2 小時保鮮期）。reducer 加 `RESTORE`；重開時自動接續並顯示「📌 已幫你接續上次選到一半的進度」＋「重新開始」出口。讀回來會**逐欄驗證合法性**，不合法整份丟掉，不讓壞 state 卡在中間步驟 |
| **P0-2** | 暫緩票推廣 | `pwaUpdateHold` 只有精靈在用，許願池、評論、回覆一樣會被倒數打斷 | 新增 `hooks/usePWAUpdateHold.ts`，套上許願池投稿、評論表單、評論編輯、回覆表單。**只在真的有東西可丟時取票**（textarea 有字／給了星等／送出中），才不會有人開著空白對話框就永遠擋住更新 |
| **P0-3** | 更新只在停手時套用 | 就算取了票，其他頁面（讀長文、點選單）仍會被 8 秒倒數打斷 | 新增 `hooks/useUserIdle.ts`：停手 20 秒（或分頁切到背景）才算閒置，倒數只在閒置時啟動。使用者還在操作時橫幅改顯示「等你忙完會自動更新，不會打斷正在做的事」 |
| **P0-4** | 六色配色的無障礙把關 | 新配色的對比度沒人量過；紅綠色系圖釘在色盲下可能難分 | 新增 `scripts/check-color-contrast.mjs`（6 色 × 4 種前景 + 8 種徽章，WCAG AA 4.5:1），掛進 `prebuild`。**抓到真問題**：`--olive-deep` 在粉(4.44)／紫(4.34)底上不到 AA → 新增 `--olive-on-note: #46521c` 專供便利貼上的行動提示 |
| **P0-5** | 引導漏斗看板 | 後端 `recoStats.funnel` 早就有每步次數，但面板只是由大到小列數字，看不出「在哪一步掉最多人」 | 新增 `lib/audienceFunnel.ts` + `admin/AudienceFunnelChart.tsx`：依流程順序畫留存長條、標出「掉最多人」的一段，並跟著 7/30 日區間切換 |

**做的過程中修掉的三個坑**：
- **草稿被自己清掉**：開啟當下寫入草稿的 effect 拿到的還是「上一輪的 state」（`RESTORE` 要下一次 render 才生效），直接寫入等於用空白初始狀態蓋掉剛讀到的草稿 → 用 `draftHydratedRef` 讓開啟後的第一次一律跳過。**preview 實測時才抓到**，單看程式碼不會發現
- **對比腳本一開始用錯指標**：把「徽章色 vs 卡片色」的亮度差當作 WCAG 非文字對比在檢查，判了 18 個假失敗。實際上徽章有 1.5px `--ink` 外框，界線靠外框而非色塊亮度差 → 改檢查「外框對徽章底、對卡片底都 ≥3:1」
- **漏斗的假流失**：學生不選職務、非行政不選處室，把這兩步塞進主漏斗會算出巨大假流失 → 主漏斗只留人人必經的 5 步，分支步驟另外列並以其上游為分母；另外強制單調遞減，避免寫入時間差造成「越走越多人」

**驗證**：`tsc`／`eslint --max-warnings 0`／**271 項單元測試全綠**（新增 25 項：草稿 8、hold 6、idle 3、漏斗 9 + 元件 4，並改寫 1 項因新行為而過時的舊測試）；`node scripts/check-color-contrast.mjs` 全數 PASS；preview 實測草稿接續、重新開始、提示條與眼罩標籤重疊修正。

### `2026-07-28` 第六批（🛡️ 引導期間不被自動更新打斷 ＋ 🎨 便利貼六色輪盤）`v3.6.106`

使用者實際使用時提出的兩個 UX 問題，都在族群引導精靈這條動線上。

| 問題 | 根因 | 修法 |
|---|---|---|
| **選族群選到一半被重新整理，結果什麼也沒選到** | 精靈作答期間，SW `updatefound` 或 `version.json` 輪詢一偵測到新版就跑 8 秒倒數自動 `reload()`；另一條 `serviceWorkerRegistration.ts` 的「偵測到 waiting worker 就靜默套用 + reload」也可能同時開火。兩條路徑都不知道使用者正在作答 | 新增 `client/src/lib/pwaUpdateHold.ts`「更新暫緩票」：`acquirePWAUpdateHold()` 取票、回傳的函式釋放。**用計數不用布林**，避免兩個流程同時開啟時先關的那個誤放行。精靈 `open` 期間持票；`PWAUpdatePrompt` 持票時 `isUpdateAvailable` 一律視為 false（不倒數、不顯示更新便利貼）；靜默套用改走 `runWhenUpdateAllowed()` 等票放完才 SKIP_WAITING。**更新不會被丟掉，只是延到選完** |
| **卡片只有左綠右藍兩色，很單調** | 身分卡與推薦卡的底色寫死 `nth-child(even)` → 奇數綠、偶數藍，一整片公佈欄只有兩色棋盤格 | 新增 `client/src/lib/noteTone.ts` 六色輪盤（綠／藍／黃／粉／橘／紫）：每輪 Fisher–Yates 洗牌後依序發，一屏 6 張不撞色，跨輪銜接也避開與前一張同色；洗牌 seed = 該批工具 id／該題選項值 → **同批重繪顏色不跳動，「換一批推薦」「換下一題」才換排列**。傾斜角與圖釘顏色跟著色調換；首頁「為你推薦」條同步套用 |

**順帶修掉的兩個小坑**：
- 揭曉動畫 `animation-fill-mode: forwards` 的收尾 `rotate(0)` 會把色調帶的傾斜角吃掉 → keyframes 收尾改成 `rotate(var(--note-tilt))`
- 卡片底色變六色後，`badge--role`（原本用 `--note-green`）會貼在綠卡片上糊掉 → 徽章底色全部調飽和一階（role/stage/discovery/universal ＋ 新鮮度徽章）

**驗證**：`tsc --noEmit` 通過；audience 相關單元測試 10 項全綠；akai-vite preview 實測身分卡黃／紫、學段卡紫／藍／橘、結果頁 6 張黃綠紫藍橘粉，按「換一批推薦」後配色順序確實改變；console 只有既有無關警告（fetchPriority、a-in-a、vite HMR socket）。

> 💡 `pwaUpdateHold` 是通用機制，之後任何「不能被打斷」的流程（許願池表單、多步驟設定）都可以直接取票保護。

### `2026-07-28` 第五批（⏳ 補完 addedAt ＋ 把「太久沒複查」從錯誤設計改對）`v3.6.105`

接 v3.6.102 留下的洞。查證後發現不是一個洞是三個：

| 洞 | 查證結果 | 處理 |
|---|---|---|
| 121 個外部工具只有 24 個有 `addedAt` | 這欄位不只理由體檢在用 —— 推薦引擎的 freshness 加分（45 天窗）與卡片「🆕 上架 N 天」徽章也吃它，**97 個工具等於永遠拿不到這些判斷** | 新增 `scripts/backfill-added-at.mjs`，用 git pickaxe（`-S'"id": N,'`）找出每個 id 第一次出現在 tools.json 的 commit。97/97 全部推導成功，抽驗 #97／#84／#61 都命中對應的「新增工具」commit。另補上 client-only 的 #100 |
| 「太久沒複查」門檻 180 天，但站上最老的工具才 155 天 | **這個訊號上線以來偵測數一直是 0**；而補完 `addedAt` 之後，55 個首版工具會在 **2026-08-22 同一天集體過期**，panel 瞬間多出兩百多條，把真正該改的 32 條洗掉 | 把 `stale` 從「問題訊號」**降級成排序與情境資訊**。一條寫得好的理由不會因為時間過去就變爛，真正會爛的是「工具改了」，那不是絕對天數測得出來的 |
| `reasonsReviewedAt` 只存在於型別裡，沒有任何流程會寫它 | 沒有蓋章機制，天數只能單向長大，複查完也歸不了零 —— 這欄位是死的 | 新增 `npm run reasons:reviewed -- <id...>`，實測 #9 從「155 天前」歸零到「0 天前」（測完已把假章拔掉，全站蓋章數維持 0） |

**改動後的樣子**：
- 體檢清單仍是 32 條（stale 本來就沒貢獻任何一條），但**排序改了** —— 同樣多問題時最久沒複查的排前面，所以現在最前面是 #37／#33／#30／#28／#9 這些二月就在的老工具
- 每條問題附「從未複查・N 天前」當情境
- Admin 面板拆成兩個檢視：「🩺 該重寫的」與「⏳ 最久沒複查 Top 20」。後者明講**不代表有問題**，只是排隊順序
- `validate-tool-pipeline.ts` 加一道 `addedAt` 守門，避免這個洞隨新工具重新打開

**驗證**：`tsc --noEmit`、`eslint --max-warnings 0`、**240 個測試**、`npm run build` 通過（含三道守門）。

> ⚠️ 前端測試有一隻抓不到的 flaky：本日出現 2 次、每次都是單一測試失敗，之後連續 9 次乾淨跑都無法重現，
> 因此**沒有確認是哪一個測試**。先記在這裡，再出現時要優先抓。
...
依 `FUTURE_OPTIMIZATION_V3.6.100.md` 選定的 A＋B＋C 三包一次完成。

| 項目 | 完成內容 | 驗證結果 |
|---|---|---|
| **P0-1** `audienceFit` 語意防呆 | `isEligible()` 的 `departments` 改成**只約束行政人員**：明寫 `teacherRoles` 的導師／科任照職務判定，不再被處室連坐；未寫 `teacherRoles` 的純行政工具維持舊語意（向後相容）。舊版有 **9 個工具**（#5 #18 #42 #46 #47 #48 #53 #72 #80）明寫了導師／科任卻被整組排除 | 9 個工具的導師全數解封；科任／行政仍依各自 `teacherRoles`／`departments` 正確過濾；新增 6 個單元測試 |
| **P0-1** 死理由偵測 | `validateAudienceFit()` 新增檢查：`reasons` 裡永遠取用不到的鍵（如 `teacherRoles` 不含 `homeroom` 卻寫了 `reasons.homeroom`）一律報錯。掃全站現況為 0 違規，可直接升級成硬錯誤 | 兩個舊測試的過時預期已更新為新規則；新增 1 組 5 案例測試 |
| **P0-1** 可觸及性檢查 | `validate-audience-fit.ts` 新增：每個工具至少要能被 1 個實際 profile 推薦到。用「逐一工具單獨丟進推薦引擎」測純資格，避免被 `upgradeToId` 家族去重誤判（#4 #13 #14 #21 是刻意被去重的舊版） | 這道檢查若早存在，#122 與 #80 兩次都會被擋下 |
| **#80 修正** | 移除 `departments`，讓導師／科任撈得到；`reasons.academic`／`student-affairs` 仍給行政人員使用 | 導師／科任／教務／總務四種身分皆可觸及 |
| **P1-6** 上架守門 | 新增 `scripts/validate-tool-pipeline.ts` + `npm run validate:pipeline`，掛進 `prebuild`／`predev`：預覽圖與 OG 圖必須有欄位且檔案存在、每個工具必須有手寫長文 `POST_N`、全 repo 不得誤寫「新明」 | 首次執行就抓到真 bug：**74 個工具的 OG 圖檔已存在但 `tools.json` 沒填 `ogPreviewUrl`**，導致 `PageHead` fallback 到 1024×1024 卡片圖 → 分享到 LINE／FB 用錯圖。已全數補齊 |
| **P0-4** 里程碑 SSOT | 新增 `lib/visitorMilestones.ts` 統一級距與比例語意（兩個計數器過去各有一份且內容不同）；`10000 → 50000` 的 5 倍斷層補上 `20000／30000`；新增「以目前速度預計 X 個月達成」 | 新增 16 個單元測試（含「級距不得一次跳超過 5 倍」的回歸測試）；實測顯示「下個里程碑・約 9 個月」 |
| **P1-2** 新鮮度徽章 | 新增 `lib/recommendationHistory.ts` 記住上次推薦過的工具；推薦卡片加第二枚徽章「🆕 剛上架／上架 N 天」「👀 上次沒看到」，與既有的「為什麼推」徽章並存 | 實測 6 張卡片正確標記；徽章間距 15.8px 無重疊、無橫向捲動 |
| **P1-1** 回訪一鍵沿用 | 回訪重問不再從頭選四步：新增 `returning` 步驟，帶出上次身分 →「✅ 還是『國小導師』」直接看新推薦／「🎯 這次想解決別的事」跳痛點頁／「✏️ 我的身分變了」重頭選。**只沿用身分不沿用痛點**（那是上次的煩惱）。使用者主動點「重選」仍走完整精靈 | 實測三個按鈕行為正確；`returning` 為第一步故隱藏「上一步」 |
| **P0-3** 成效量測 | GA 埋點全面帶 `mode=first_time／re_prompt`：`audience_wizard_opened`／`audience_wizard_completed`（新）／`audience_wizard_dismissed`（新，帶跳出步驟）／`audience_reco_impression`／`audience_reco_click`；另有 `audience_wizard_returning_choice` 記錄三選一。**不動 Firestore callable 契約**，純 GA | 可回答「重問完成率 vs 首次」「重問 CTR」「在哪一步跳掉」「7 天間隔該不該調」 |

**驗證**：`tsc --noEmit`、`eslint client/src scripts --max-warnings 0`、**224 個單元測試全綠**、`npm run build` 通過（含兩道新守門）。
...
| 項目 | 完成內容 |
|---|---|
| Firestore 權限 | `analytics`、`visitorStats`、`toolUsageStats` 禁止 client 直寫；原公開分析事件改走 `recordPublicAnalytics` callable，逐類型／欄位白名單驗證後由 Admin SDK 原子累計。後台原始分析資料只允許 Admin custom claim 讀取。 |
| 匿名與 Admin 登入 | 公開分析寫入前自動建立匿名 Firebase 身分；Admin 畫面不再把匿名 session 誤認為已登入帳號，Google 登入取消／失敗會顯示錯誤。主動登出後，本次 session 不會被立即匿名登入。 |
| 許願池可信度 | 移除讀取失敗時假裝空陣列與更新／刪除永遠回傳成功的行為；只有 Firestore 寫入成功才更新 UI 與顯示成功訊息，失敗時保留原狀。Rules 增加許願與 error log schema 白名單。 |
| 移除示意數字 | 地理城市圖不再顯示硬編碼數字；本機點擊熱力圖明確標示只代表目前 Admin 瀏覽器。 |
| 備份還原語意 | 後台與 Cloud Function 統一改稱「核心頂層資料快照／合併還原」；預演與結果會列出快照文件、現有文件、保留的額外文件，並明示不刪除快照外文件與子集合。 |
| 儀表板拆分 | `AnalyticsDashboard.tsx` 從約 1,900 行降至約 1,050 行，拆出行為分頁、基礎設施面板與統計卡；熱力／日曆分頁改為需要時才載入。Web Vitals 七天查詢改平行載入，修正 CLS 小數與 p75 計算。 |

**🧪 驗證與部署**

- `npm run check`、functions TypeScript build、正式版 `npm run build` 通過。
- 匿名登入、Admin 狀態與許願池後台操作共 20 項針對性測試通過。
- `recordPublicAnalytics` 與 Firestore Rules 已部署至 `akai-e693f`；正式環境匿名 callable 邊界與偽造許願／直接竄改分析資料均實測遭拒，測試匿名帳號已清除。
- Rules Emulator 測試檔與指令已補齊；本機因尚未安裝 Java 無法啟動 Emulator，但同一份 Rules 已通過 Firebase 部署端編譯。
- 全專案既有測試另有 12 項舊失敗（工具數寫死 114、TooltipProvider 測試包裝、舊排序預期與空測試檔），不屬於本次變更。

### `2026-07-11`（🎯 客群推薦體驗、成效追蹤與定位回饋優化）

**✅ 已完成**

| 項目 | 完成內容 | Commit |
|---|---|---|
| 換一批推薦回頂 | 新一批推薦完成渲染後，平滑定位到第一張新卡；尊重減少動態效果設定。 | `a001feb` |
| 完整匿名客群分析 | Admin 新增精靈流程／離開位置、客群與痛點選擇分布、推薦版位成效與痛點點擊關聯。只寫入匿名加總，不保存帳號或個人選擇紀錄。 | `0bcdcef` |
| 首頁推薦卡定位修正 | 定位選擇器限定為下方 `.bulletin-tool-card`，避免首頁推薦列與工具卡共用 `data-tool-id` 時捲回原地。 | `941dd3f` |
| 目標工具卡聚光提示 | 定位後顯示黃框、聚光脈衝、紅色虛線圈與「你正在找的推薦工具」標籤；提示延長為 6 秒，連續點擊會重設計時。 | `abd912d`、`15a80ef` |
| P0-D 推薦定位 RWD 驗收 | 定位時顯示進度與重試、逾時明確告知；手機標籤置中且不溢出；減少動態效果仍保留清楚高亮。新增 Pixel 5 Playwright 驗收，量測目標卡與標籤都在可視範圍。 | 本次 |
| P0-C 漏斗轉換與異常提醒 | 新增每日漏斗、每日客群 CTR、首次／換批推薦成效；Admin 顯示精靈完成率、換批 CTR 與樣本達 20 次後的近 7 天 CTR 下滑警示。 | 本次 |
| P0-E 推薦資料健康檢查 | 驗證所有主要客群至少 6 張有效推薦，輸出各痛點覆蓋數，並警示未覆蓋或少於 3 項的痛點。 | 本次 |

**🔎 驗證**

- `npm run check` 通過。
- 客群推薦相關測試共 47 項通過。
- ESLint 目前受專案既有 ESLint 9 與 `.eslintrc` 設定不相容影響，未作為本次阻擋條件。
